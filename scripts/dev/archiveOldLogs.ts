...
async function archiveActivityLogsToFile(
  ...
  // Find logs to archive — fetch both conditions and merge
  const cutoffs = {
    regular: cutoffDate.toISOString(),
    admin: adminCutoffDate.toISOString(),
  };
  ...
  const { data: regularLogs, error: regularError } = await supabase
    ...
    .select('*')
    .neq('category', 'ADMIN')
    .lt('timestamp', cutoffs.regular) as any;
  ...
  const { data: adminLogs, error: adminError } = await supabase
    ...
    .select('*')
    .eq('category', 'ADMIN')
    .lt('timestamp', cutoffs.admin) as any;

  if (adminError) throw adminError;

  const logsToArchive = [...(regularLogs || []), ...(adminLogs || [])];
...
async function archiveBattleLogsToFile(
  ...
  const { data: logsToArchive, error } = await supabase
    .from('battle_logs')
    .select('*')
...
async function cleanupOldBattleLogs(battleRetentionDays: number): Promise<number> {
  ...
  console.log(
    `[Cleanup] Deleted ${count || 0} battle logs older than ${battleRetentionDays} days`
  );
...
async function previewCleanup(config: ArchiveConfig): Promise<{
  ...
  const cutoffs = {
    regular: activityCutoffDate.toISOString(),
    admin: adminCutoffDate.toISOString(),
  };

  const { count: activityRegularCount, error: regularErr } = await supabase
    .from('activity_logs' as any)
  ...
  const { count: activityAdminCount, error: adminErr } = await supabase
    .from('activity_logs' as any)
    .select('*', { count: 'exact', head: true })
    .eq('category', 'ADMIN')
    .lt('timestamp', cutoffs.admin) as any;
  ...
  const activityCount = (activityRegularCount || 0) + (activityAdminCount || 0);

  // Count battle logs
  const battleCutoffDate = new Date();
  battleCutoffDate.setDate(battleCutoffDate.getDate() - config.battleRetentionDays);

  const { count: battleCount, error: battleErr } = await supabase
    .from('battle_logs')
    .select('*', { count: 'exact', head: true })
    .lt('timestamp', battleCutoffDate.toISOString());

  if (battleErr) throw battleErr;

  return {
    activityLogs: activityCount,
    battleLogs: battleCount || 0,
    total: activityCount + (battleCount || 0),
  };
}
 ...
 *    - Use Supabase batch operations for large deletions
