...
async function getBotCountByZone(): Promise<Record<number, number>> {
  ...
  (bots || []).forEach((bot: any) => {
    const zone = bot.bot_config?.zone ?? 0;
...
async function getBotCountByNest(): Promise<Record<number, number>> {
  ...
  (bots || []).forEach((bot: any) => {
    const nestId = bot.bot_config?.nest_affinity;
...
async function spawnBots(
  ...
  for (let i = 0; i < actualCount; i++) {
    ...
    if (concentrationPosition && targetZone === undefined && nestAffinity === undefined) {
      ...
      const nestPos = getRandomPositionNearNest(nestAffinity, 10);
      if (nestPos) {
        position = nestPos;
        zone = calculateZone(position);
      } else {
      ...
      if (Math.random() < 0.20) {
        ...
        const nestPos = getRandomPositionNearNest(affinity, 10);
        if (nestPos) {
          position = nestPos;
          zone = calculateZone(position);
        }
  ...
  if (botsToInsert.length > 0) {
    const { error: insertError } = await supabase.from('players').insert(botsToInsert);
