...
| 86 | **FID-20260917-009 â Stripe VIP remediation implemented (queue-jumped):** money path re-keyed to `players.username` in all four functions (grantVIP/revokeVIP/extendVIP/checkVIPStatus) + `getUserByStripeCustomerId` alias now projects username as id (was the NULL-mongoId alias with a fallback that existed only because mongoId was empty). Webhook false-success fixed at **three** sites â checkout grant=false now throws and does NOT record the payment (â 500 â Stripe retries); the renewal and cancellation pathsâ catch-and-swallow blocks were removed (reading the route for the fix revealed they would have eaten the new throws). All five event handlers moved to `lib/stripe/webhookHandlers.ts` (route = transport + signature verification only; Next route modules may only export HTTP verbs â the first export attempt failed the generated route-type check). Evidence: 11 pins (6 service WHERE-key assertions via real drizzle, 5 webhook behavioral), live probe 5/5 on a `mongoId: null` clone of fame (the audited pg-era cohort shape) â grant/extend/revoke/check all true, isolation held, zero residual rows; gates tsc 0, eslint 0 (6 files), vitest **1015 passed** (+11), census exit 0. | 2026-09-17 (session 051) | Closed (implemented loop-complete; dev/fids/FID-20260917-009-stripe-vip-username-keying.md â Â§8 hash fills on commit) |
| 87 | **Product-remainder survey (post dead-route/shrine/territory arcs):** the new P0 is chat UI that lies â ChatMessage report/block/delete are Task-8 stubs showing success toasts with no backend (report/block have zero persistence anywhere; deleteâs route EXISTS and ChatPanel calls it for itself but never passes onDelete into ChatMessage). P1: AlliancePanel (complete UI + five live /api/clan/alliance routes) is mounted nowhere â invisible diplomacy system. P2: chat polish cluster (profile modal, item-details modal, MessageThread real-time, ChatPanel backlog) + the queued tutorial /complete deletion (route alive, zero client callers, evidence-complete). Survey: dev/audits/PRODUCT-REMAINDER-SURVEY-2026-09-17.md; recommended order: chat honesty FID, alliance mount FID, tutorial deletion micro-FID. | 2026-09-17 (session 052) | Closed (survey recorded; P0/P1 FIDs proposed, awaiting go-ahead) |
