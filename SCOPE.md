...
**Last updated:** 2026-09-25 — FID-20260925-002 closed on `9a5ba11` + `2e1d63b`: ledger census check D makes a session record a precondition of every closure from 2026-09-24 forward (row 124 option (a), authorized by "resume"); row 126 added and row 124 annotated; `SESSION-2026-09-25-001.md` records the session; CHANGELOG/VERSION 0.0.37. *(Earlier the same day: FID-20260925-001 closed on `9b7125e` — the four 2026-09-24 gate closures retro-filed as `SESSION-2026-09-24-002.md`; rows 123-125; 0.0.36.)* *(This field has now gone stale twice in six days: it read "2026-09-14 (session 005)" while rows 112-117 were recorded on 2026-09-19/09-23, and then read "rows 116-117 … CHANGELOG 0.0.32" while rows 118-122 and releases 0.0.33-0.0.35 existed — reported as a finding, not only corrected (FID-20260925-001 §2 finding 6; row 124). Prior content preserved: 2026-09-24 — FID-20260923-001 and FID-20260923-002 closed on their committed hashes and archived; SCOPE rows 116-117 dispositioned; CHANGELOG 0.0.32.)*
...
| 11 | `lib/territoryService.ts` income double-collection dedup can never trigger: `db.execute` on node-postgres drizzle resolves to a pg `QueryResult` (rows under `.rows`), so the `.length > 0` guards evaluate `undefined > 0` → always false — a second same-day `collectDailyTerritoryIncome` call re-collects income. Same dead-guard class affects the duplicate-claim checks in `claimTerritory`/`validateTerritoryClaim` (they rely on jsonb `JSON_CONTAINS` raw SQL). Fix changes runtime behavior — owned by the DB-direction decision (FID-20260902-001) | 2026-09-02 (SESSION-2026-09-02-009) | Closed (2026-09-17 work-order item 5: repair ALREADY SHIPPED in the FID-20260917-001 treasury-lock rework - the dedupe now re-checks the locked row's lastTerritoryIncomeCollection under withClanTreasuryLock, no QueryResult shape is read at the site; JSON_CONTAINS claim checks live on typed builder queries; repo-wide sweep of the .length-off-QueryResult class finds zero live instances) |
...
| 83 | **FID-20260917-008 — /api/player/inventory migrated off the Mongo stack:** the route InventoryPanel fetches every game load 401'd since the pg pivot (dead `playerId`-cookie auth + `clientPromise` Mongo query; the panel's `if (response.ok)` guard hid it silently). Rewritten on pg: `requireAuth` (darkframe_session) → 8-column select → the EXACT unwrapped InventoryData contract (numerics parsed, expiresAt ISO-string; legacy `resources`/`equipment` dropped — consumed by no client). First client-facing instance of the row-#24 disease. 5 pins; live probe 3/3 exit 0 (premise: fame row 47 items / cap 2000 / diggers 25+23 / bonuses 39·0·37·0); live server 200 with real cookie; gates tsc 0 / eslint 0 / vitest 1004+1skip. | 2026-09-17 (session 048) | Closed (commit `90f7f5f`; FID archived; CHANGELOG [0.0.9]) |
| 84 | **Mongo-residue census (row #24) — the shim is 1,554 lines, not a shim:** `lib/mongodb.ts` is a full Mongo-over-pg emulation layer (clientPromise = resolved compat client; filters/updates/aggregate translated to drizzle or in-memory JS) — the data is already in Postgres, row #24 is a translation-layer retirement, not a data move. Census: 48 importer files in 4 classes — 15 real shim consumers (incl. the stats global $group and clan leaderboard aggregating in-memory over full tables), 13 dead `await clientPromise; const _db = ...` fetches (dm/friends/tutorial — underscore-unused, zero-risk deletes), ~20 connection-helper-only consumers (mechanical swaps), 2 out-of-app stragglers (validate-referrals-cron still `new MongoClient(MONGODB_URI)` against the dead DB; type-only ObjectId refs). Clustered migration plan A→B→C→D with the double-run oracle for aggregates recorded in `dev/audits/MONGO-RESIDUE-CENSUS-2026-09-17.md`; 3 decisions requested (ActionLog/adminLogs table mapping, cron disposition, sequencing). Zero code changes. | 2026-09-17 (session 049) | Closed (audit: dev/audits/MONGO-RESIDUE-CENSUS-2026-09-17.md; migration FIDs to follow per cluster) |
| 85 | **Stripe VIP × mongoId audit — the grant path is dead for 100% of players:** cohort probe (66 players): mongo_id populated for ZERO (nothing writes it post-pivot, no backfill ever ran), while all four money functions (grantVIP/revokeVIP/extendVIP/checkVIPStatus) key lookups on `eq(players.mongoId, userId)` — and checkout metadata embeds **username** as userId. verify-session at least 500s loudly to the customer who just paid; the **webhook logs "VIP granted successfully" on the false return** and records the payment anyway — the silent loss. No incident history only because paymentTransactions has 0 rows (no completed checkout in dev yet). The lone vip=1 (fame, 2027) is the admin endpoint's work (mod_log VIP_GRANT trail) — admin + referral VIP paths are healthy. Remediation proposed: delete the indirection (lookup by username, no backfill fiction) + fix the webhook false-success + pins; jump Cluster D's queue — this is the only guard between every future paying customer and a silent loss. | 2026-09-17 (session 050) | Closed (audit: dev/audits/STRIPE-MONGOID-VIP-AUDIT-2026-09-17.md; remediation FID recommended, queue-jumped) |
| 86 | **FID-20260917-009 — Stripe VIP remediation implemented (queue-jumped):** money path re-keyed to `players.username` in all four functions (grantVIP/revokeVIP/extendVIP/checkVIPStatus) + `getUserByStripeCustomerId` alias now projects username as id (was the NULL-mongoId alias with a fallback that existed only because mongoId was empty). Webhook false-success fixed at **three** sites — checkout grant=false now throws and does NOT record the payment (→ 500 → Stripe retries); the renewal and cancellation paths’ catch-and-swallow blocks were removed (reading the route for the fix revealed they would have eaten the new throws). All five event handlers moved to `lib/stripe/webhookHandlers.ts` (route = transport + signature verification only; Next route modules may only export HTTP verbs — the first export attempt failed the generated route-type check). Evidence: 11 pins (6 service WHERE-key assertions via real drizzle, 5 webhook behavioral), live probe 5/5 on a `mongoId: null` clone of fame (the audited pg-era cohort shape) — grant/extend/revoke/check all true, isolation held, zero residual rows; gates tsc 0, eslint 0 (6 files), vitest **1015 passed** (+11), census exit 0. | 2026-09-17 (session 051) | Closed (commit `70e6b2d`; FID archived; CHANGELOG [0.0.9]) |
| 87 | **Product-remainder survey (post dead-route/shrine/territory arcs):** the new P0 is chat UI that lies — ChatMessage report/block/delete are Task-8 stubs showing success toasts with no backend (report/block have zero persistence anywhere; delete’s route EXISTS and ChatPanel calls it for itself but never passes onDelete into ChatMessage). P1: AlliancePanel (complete UI + five live /api/clan/alliance routes) is mounted nowhere — invisible diplomacy system. P2: chat polish cluster (profile modal, item-details modal, MessageThread real-time, ChatPanel backlog) + the queued tutorial /complete deletion (route alive, zero client callers, evidence-complete). Survey: dev/audits/PRODUCT-REMAINDER-SURVEY-2026-09-17.md; recommended order: chat honesty FID, alliance mount FID, tutorial deletion micro-FID. | 2026-09-17 (session 052) | Closed (survey recorded; P0/P1 FIDs proposed, awaiting go-ahead) |
| 88 | **FID-20260917-010 — unreachable clan UI mounted (Alliances tab) + FID-012 research tab unblocked:** AlliancePanel (complete diplomacy UI + five live /api/clan/alliance routes) had zero importers — now mounted as ClanPanel’s Alliances tab (Handshake icon, five-prop contract honored: clanId/playerId/role/clanName/treasuryMetal, the last riding the sanctioned /api/clan/[id] payload via rowToClan’s bankTreasuryMetal mapping). Mount-surface probe also found FID-20260916-012’s ClanResearchPanel mounted behind a bare `disabled` TabButton — dead on arrival; flag removed. First mount-level render pins in the house (real ClanPanel under jsdom, GameContext+fetch mocked; fixture from ClanPanel’s actual read set). Gates: 3 pins, tsc 0, eslint 0, census 0, suite 1020+1skip. | 2026-09-17 (session 053) | Closed (commit `296b48c`; FID archived; CHANGELOG [0.0.9]) |
| 89 | **FID-20260917-011 — seven-item work order (the full pending ledger, approved in one pass):** census gate + FID-010 batches land; triple closure (FID-008/009/010: §8 hashes, archives, SCOPE 83/86/88 flips, CHANGELOG 0.0.9); chat-honesty FID (delete wire-through + report persistence + GLOBAL block per operator decision); tutorial /complete deletion micro-FID; row 11 territory dedupe repair (.rows.length guard); referral decoy deletion (the cron validates Mongo while pg-side checkReferralValidation has zero callers — follow-up FID candidate: wire pg validation on-login) + rows 22/23 stale flips. Evidence probed in-loop (Law 16); execution order normative; per-item gates. | 2026-09-17 (session 054) | Closed (2026-09-17: all items executed - item 0 233bbca, 1 296b48c, 2 85a03b4, 3 d89ac93, 4 d51992f, 5 af7afae, 6 7de2230, 7 this commit; per-item evidence in each commit + session 054) |
| 90 | **FID-20260917-012 - chat honesty (report + GLOBAL block) implemented:** chat_reports table + POST /api/chat/report (validated reasons, self-report refusal); blocked_users table + blockService (idempotent block, self-block refused) enforced server-side in chatService (message filter via viewerId) and messagingService (conversation filter); admin visibility via GET /api/admin/moderation?type=reports; honest Report/Block buttons on live ChatPanel rows; orphan ChatMessage stubs wired to real endpoints. 12 pins; block-route auth retagged (ok-discriminant) so handlers cannot infer an undefined fall-through under exactOptionalPropertyTypes. | 2026-09-17 (session 054) | Closed (2026-09-17: closed on d89ac93; archived; CHANGELOG 0.0.10) |
| 91 | **FID-20260917-013 - tutorial /complete route deleted:** dead Mongo-era endpoint removed; zero client callers re-verified at execution (app/ components/ lib/ hooks/ context/ utils/); inverted route census exit 0 post-deletion. | 2026-09-17 (session 054) | Closed (2026-09-17: closed on d51992f; archived; CHANGELOG 0.0.10) |
| 92 | **Follow-up candidate - wire pg referral validation on login:** the Mongo-era cron decoy (scripts/validate-referrals-cron.ts + npm validate-referrals + guide crontab/Vercel sections) was deleted after evidence showed it validated against Mongo while the real pg logic (referralService.checkReferralValidation, 7-day + 4-login criteria) has zero callers; nothing validates pg referrals today. Proposed shape: hook validation into the existing on-login streak/login-tracking path. | 2026-09-17 (session 054, work order item 6) | Closed (2026-09-17: candidate implemented as FID-20260917-014, tracked as row 93) |
| 93 | **FID-20260917-014 - referral validation on login (row 92 candidate) implemented:** processLoginReferralEvents(username) hooks POST /api/auth/login (replacing the dead Mongo-era lastActive write - silent no-op since the pg pivot); maintains referrals.loginCount/lastLogin (previously written only at referral creation, making the 4-login criterion unreachable by construction) and auto-validates pending referrals meeting 7d + 4 logins through the existing reward path; exclusions: invalidated + abuse-flagged (D2); validateReferral's validated flip is now a conditional claim (UPDATE WHERE validated=0 RETURNING) so concurrent logins or racing admin validation cannot double-pay (D3). 8 pins; live probe 10/10 against the real dev DB (8-day seeded referral: fires 1-3 unvalidated, fire 4 validates + credits referrer rewards, fire 5 idempotent; snapshot/restore cleanup). | 2026-09-17 (session 055) | Closed (2026-09-17: closed on 1bd818b; archived; CHANGELOG 0.0.11) |
| 94 | **FID-20260917-015 - Cluster B batch 1 (stats, check-name, tutorial x2) migrated to pg:** fresh census shows 73 lib/mongodb importers and 15 direct clientPromise users (audit's 13 was stale); the census's trivial slice rewritten contract-pinned: stats gets SQL orderBy(...).limit(10) (the noted perf win) + one SQL aggregate + COUNT probes, with totalPower DERIVED as totalStrength+totalDefense (total_power is not a pg column - D1, matches rankingService:123); check-name uses lower() equality (kills the user-input RegExp seam; case-insensitivity load-bearing); tutorial eligibility read + restart delete on pg tutorial_progress; decline theater removed. 8 pins; live HTTP probe 16/16 vs real server+DB (SQL oracles, case-mangled clan name, session-identity restart). Batch 2 queue: friends x2, DM x3, ban-player, clear-flags, logs/cleanup, build-unit, antiCheatDetector. | 2026-09-17 (session 056) | Closed (2026-09-17: closed on 81a4d02; archived; CHANGELOG 0.0.11) |
| 95 | **FID-20260917-016 - Cluster B batch 2 (friends/DM family + ban-player, clear-flags, logs/cleanup, build-unit) migrated to pg:** 6 theater files (friends x2, dm x3 - _db assigned, never used) cut to pg-native services; 4 real rewrites with schema-truth fixes: bans insert generates its no-default 24-char id + NOT NULL createdAt with smallint flags; unban clears the five real ban columns (D4 - players has no unbannedAt/unbannedBy, the Mongo $set mapped to nothing); autoResolveFlags writes resolved=1 with resolver evidence in metadata jsonb (D5 - no resolvedBy/resolvedAt/adminNotes columns exist); audit rows land in mod_log (D1 - adminLogs was an unmapped shim name matching nothing); cleanup dry-run mirrors the real deleter's single cutoff over player_activity (D2 - no category column, the ActionLog activity/admin split was fiction); build-unit preserves shim charge semantics as SQL deltas on the flat resource columns, jsonb append shape unchanged, and lands the schema-contracted investedMetal/investedEnergy deltas (D3 - the Mongo path never maintained the ledger). 20 pins; live probe 26/26 vs real handlers+DB (ban/unban/flags/cleanup + build via fresh register-route session on its own factory; probe-owned cleanup, residue zero). Gates: tsc 0, eslint 0, census 0 live clientPromise under app/api, suite 1068 (+20). | 2026-09-17 (session 058) | Closed (2026-09-17: closed on 5496fbf; archived; CHANGELOG 0.0.11) |
| 96 | **Agent-granted deferral of 17 relative-import lib services — Law 2 violation corrected (operator-directed):** FID-20260917-017's scoping sweep surfaced 17 lib services riding relative `./mongodb` imports (statTrackingService, movementService, auctionService, achievementService, beerBase*, bot*, dailyLoginService, discoveryService, mapGeneration, tierUnlockService, cacheWarming, combatPowerService) plus `lib/shrineServer` + `lib/websocket/chatHandlers` (1 op each). The FID originally filed them under "Deliberate deferrals (recorded, not scoped)" — a deferral granted by the agent inside a scoping document, without a blocking presentation to the operator and without a SCOPE.md entry, in violation of Law 2 ("Deferring approved work without presenting" / scope reduction is a silent decision) and the Step-Level Anti-Deferral table (`deferred` = Operator only). Mitigating fact: FID-017 is a scoping FID stopped at `scoped — awaiting operator batch selection`; no batch executed, no code touched, nothing dropped in practice — overreach in the proposal, not in execution. Correction (operator-approved amendment, 2026-09-18): FID-017 §2.I recast as "Active scope items awaiting operator batch selection" (supersedes the deferral language, amendment note in place), slice 6 re-labeled as active items with operator batch selection; this row is the ledger entry the protocol requires. Batch selection for the 17 + 2 trivial files remains a blocking presentation to the operator. | 2026-09-18 (this session) | Closed (2026-09-19): operator selected all slices + batch 4 — 5 route slices (`fb5b702`…`dc7c36c`, app/api at zero shim importers) and all 17 services + 2 trivia + barrel (`c9c3189`…`bcffa73`) landed gates-green; FID-017 §8 carries the chain, archive in dev/fids/archive; pre-push Gate 3 (shim runtime census) wired so the class can't return. Residual: 2 scripts/ importers recorded in §8 for a follow-up. |
| 97 | **Shim-exit — lib/mongodb.ts deleted outright (follow-up to row 96's residual):** the quarantined shim, its packages (mongodb + mongodb-memory-server), and everything that existed to serve or verify it are removed: collection census (script + pins), shim-semantics pins, verifyShimSemanticsLive, the opt-in friends live-DB suite, vitest setup's TEST_MONGO_MEMORY block. Two scripts converted (spawnBots: mapDomainPlayerToRow insert, phantom isBot cooldown dropped; archiveOldLogs: full drizzle rewrite, admin-tier retention mirrored to live single-window semantics). server.ts + 13 already-pg-native scripts took import swaps. Pre-push Gate 3 rewritten as total eradication: any mongodb module/package/env reference outside __tests__ refuses the push (planted-violation verified). Chain `f827655` → `30f694e` → `1f66c5a`; gates at close: suite 114/1141, tsc 0, eslint clean, census 0. Provenance comments naming the shim kept (accurate history). | 2026-09-19 (this session) | Closed (2026-09-19: closed on 1f66c5a; CHANGELOG 0.0.13) |
| 98 | **FID-20260919-001 — auction unit-listing honesty:** the product survey's P0 refined under grounding: delivery was honest (FID-20260914-003 escrow owns the snapshot) but display stats were client-fabricated (every modal listing showed Str 100/Def 50) AND the modal's unit flow could never succeed (no unitId — rejected server-side since escrow). Fixed: service overwrites stored unitStrength/Defense/unitType from the escrowed unit (client fields never trusted); modal replaced the synthetic type-only picker with a real picker over the seller's units + post-escrow player refresh; card displays snapshot-first with legacy-scalar fallback (lib/auctionDisplay). Out-of-scope defect found live and fixed: the doc-bridge leaked raw booleans into pg smallint mirrors — every listing INSERT 500ed since batch 4 (`invalid input syntax for type smallint: "false"`); booleans now coerce unconditionally in syncAuctionDocFields. 6 pins; live probe 15/15 (create/read/cancel round-trip, escrow truth, residue zero). | 2026-09-19 (this session) | Closed (2026-09-19: closed on 123d5e2; archived; CHANGELOG 0.0.14) |
| 99 | **FID-20260919-002 — chat real-time client wiring:** subscriptions wired for `chat:message` / `chat:typing_start` / `chat:typing_stop` / `chat:online_count` / `chat:message_deleted` via a pure transition module (lib/chatSocketWiring) with poll-identical semantics (id-dedupe, self-filter, per-channel maps); typing emits socket-first with HTTP fallback and a real stop signal (was a no-op). Live probe (12/12 on tsx server.ts) exposed and fixed four server-side defects: connection listeners registered AFTER async setup (emits silently dropped in the window — reordered); delete route's dead notify path wired via getIO() + globalThis bridge (webpack route bundles got their own module copy with io=null); stale S2C type map corrected to server truth and the dead parallel chat layer deleted; sender-exclusion verified live. 16 pins; full gates green. | 2026-09-19 (this session) | Closed (2026-09-19: closed on d1b390b; archived; CHANGELOG 0.0.15) |
| 100 | **FID-20260919-003 — dead-UI generation archived:** the survey's 13 unmounted components (~3,151 lines) + orphaned HarvestButton.test moved to dev/archives/2026-09-19-dead-ui/ (census re-probed fresh: barrel-only references, dynamic-import swept; HarvestStatus hits were the service fn + TileHarvestStatus, false friends). Barrel lost 9 exports + 2 stale comments; StatsPanel.test lost two vestigial mocks of modules StatsPanel never imported. Bonus: the slice-2 harvest-caller pin was reading the archived dead component as evidence — corrected to the real live callers (HarvestModal + game page). BalanceIndicator archived with the P1 caveat recorded in the archive README; balanceService untouched. Gates: 116/1163, tsc 0, eslint clean, census zero. | 2026-09-19 (this session) | Closed (2026-09-19: closed on 9a46891; FID archived; CHANGELOG 0.0.16) |
| 101 | **FID-20260919-004 — DM real-time:** grounding refined the premise — the messages page already subscribed to every DM event but NO live path emitted any of them. Fixed three layers: (1) broadcast seams in sendDirectMessage + markMessagesAsRead via new lib/messagingBroadcast (payload mappers + personal-room fan-out, single-source room convention, getIO-null-safe); (2) 5 dead-addressing sites in messagingHandlers fixed — the socket path emitted to user_${id} rooms no client ever joins (real convention user:<id>); (3) MessageThread wired — real typing emits (were commented TODOs), incoming-message append (own-echo skip + id dedupe), read receipts flipping sent ticks. 7 pins; live probe 11/11 (HTTP send → socket delivery, typing, receipts, residue zero; one stale-server restart lesson recorded). Gates: 117/1170, tsc 0, eslint clean. | 2026-09-19 (this session) | Closed (2026-09-19: closed on 4362e82; FID archived; CHANGELOG 0.0.17) |
| 102 | **FID-20260919-005 — ask-veterans false-success fix:** the newbie-help chain was dead at every hop while the UI reported success — the client POSTs the HTTP route (the working socket handler was unreachable, emitted by nothing), the route broadcast nothing (its "Task 3" TODO), sendVeteranNotification built an object and returned it, chat:veteran_notification had zero subscribers, and the success toast read notifiedCount which the response never carried ("Notified undefined veteran players"). Fixed with lib/veteranBroadcast as the single seam ridden by both the HTTP route (getIO globalThis bridge) and the socket handler; client honesty gate (res.ok/success check, real count, server errors surfaced on 403/429); veteran-side subscription delivers a 30s toast. 7 pins; live probe 8/8 (exact payload truth incl. TTL=5min, non-veteran exclusion, real-429 twin; listener-arm-before-ask lesson recorded in §8). Gates: 118/1177, tsc 0, eslint clean. | 2026-09-19 (this session) | Closed (2026-09-19: closed on cdfaee7; archived; CHANGELOG 0.0.18) |
| 103 | **FID-20260919-006 — chat polish, live-nav + corpse annex:** grounding rewrote the premise — ChatMessage.tsx was dead code (zero importers; the survey's two "tiny gaps" were TODOs in a corpse, missed by the FID-003 census behind the chat barrel re-export, and the barrel itself was imported by nothing). Implemented on the live surface: ChatPanel's inline message-header username is now a real button navigating to /profile/<username> (useRouter added). ChatMessage + chat/index.ts archived to dev/archives/2026-09-19-dead-ui with annex; chat item-linking recorded as the actual feature gap (live-renderer parsing + AuctionHousePanel name-search/deep-link — the auction modal has no name search and is a game-page modal, not a route). Gates: 118/1177, tsc 0, eslint clean. | 2026-09-19 (this session) | Closed (2026-09-19: closed on 73754dc; archived; CHANGELOG 0.0.19) |
| 104 | **FID-20260919-007 — factory Manage tile button (operator decision: rewire to unit-factory):** the commented TODO button pointed at the never-shipped /game/factory-management and referenced a router binding that never existed (real binding was unused _router) — a naive uncomment would not compile. Mounted owner-only on factory tiles navigating to /game/unit-factory; ownership guard mirrors the live username comparison (Factory.owner: string|null; ownerless factories never match). Collateral: components barrel still re-exported the deleted chat barrel from FID-006 (latent tsc break vitest cannot catch — recorded honestly); repaired to a direct ChatPanel re-export. Gates: 118/1177, tsc 0, eslint clean. | 2026-09-19 (this session) | Closed (2026-09-19: closed on a266d5f; archived; CHANGELOG 0.0.20) |
| 105 | **FID-20260919-008 — chat item links + auction name-search + deep-link:** grounding exposed the feature's foundation as fiction — validateItem was a stub returning false for everything ("items table not in schema" TODO) and no items table exists; the real catalog IS UNIT_CONFIGS + ResourceType. New lib/catalogService (case-insensitive validation + kind resolution) with chatService delegating (route unchanged, now truthful); new lib/chatItemLinks pure parse/resolve module pinned (bounded regex, invalid brackets stay literal, links in all channels); ChatPanel renders [names] as deep-links to /game?market=<name>; auction list route + getAuctions gain an ILIKE name filter over the listing doc's item identity (LIKE wildcards stripped); AuctionHousePanel gains an Item Name search box + mount deep-link consumption with tab preselection (useSearchParams added fresh — the FID's initial precedent claim was corrected pre-implementation). 14 pins; live probe 8/8 (self-validating filter vs a live listing). Gates: 119/1191, tsc 0, eslint clean. | 2026-09-19 (this session) | Closed (2026-09-19: closed on ba16ab4; archived; CHANGELOG 0.0.21) |
| 106 | **FID-20260919-009 — tradeable listings complete (design-first, decisions D1b/D2/build):** grounding reframed the directive — no items table is warranted (a tradeable IS its inventory instance; procedurally named per cave harvest); the real gap was that trading was blocked outright (creation refused, transfer branch an empty TODO since FID-20260914-003). Shipped: pure escrow planner pinned 9/9 (whole-instance selection, no partial escrow, refund mapping), prohibition lifted with server-derived per-instance snapshots, delivery mints fresh buyer instances preserving identity, cancel/expire refund originals, modal tradeable picker (quantity cap 20), real names on cards/notifications/search/chat links, placeholder retired. Two live-probe defects fixed pre-landing: typeless delivered rows (snapshot dropped type/bonusPercent/foundAt) and id-less refund rows (itemId never mapped back). Gates: suite 1200/1200, tsc 0, eslint clean; live probe 14/14. | 2026-09-19 (this session) | Closed (2026-09-19: closed on 08410d6; archived; CHANGELOG 0.0.22) |
| 107 | **FID-20260919-010 — RP packages checkout (design reused the fixed VIP plumbing):** the shop's dead "Stripe integration pending" button now creates real one-time Stripe sessions; single-source package map (client sends only packageId — server owns price + RP), mode=payment inline price_data, webhook RP branch with the FID-20260917-009 law (idempotency probe on stripeSessionId, server-map resolution, username-keyed grant, grant-before-ledger, throw-on-false), ledger tier rp:<packageId>, VIP dispatch guard. Pins 15 (behavioral + real-module keying); live probe 11/11 incl. a real test-mode session and signed-event replay semantics; suite 1215/1215, tsc 0, eslint clean. | 2026-09-19 (this session) | Closed (2026-09-19: closed on 89cc34b; archived; CHANGELOG 0.0.23) |
| 108 | **FID-20260919-011 — hygiene micro-batch (ChatPanel doc truth + notifications disposition):** both stale docblocks rewritten (the "WebSocket placeholders" fiction that fed three survey premises is gone; virtualization kept as the one honest TODO); notifications-stack premise overturned with evidence (writer existed but was unreachable: zero trigger callers, no email sender, no readers, 0 rows in all three tables) — schema defs, re-exports, barrel line, and alertService deleted, migration 0035 idempotently drops the tables, wmd_alerts retained (live admin-health reader). Gates: suite 1215/1215, tsc 0, eslint clean. | 2026-09-19 (this session) | Closed (2026-09-19: closed on 6f08696; archived; CHANGELOG 0.0.24) |
| 109 | **FID-20260919-012 — TODO/placeholder census (doc-truth batch + two real defects):** 91 hits triaged with probes, not grep counts. Real defects fixed: DELETE /api/chat was a false-success stub (returned "Message deleted" without deleting) — now admin-gated (moderationService.isAdmin) soft-delete via the previously-unused chatService.deleteGlobalChatMessage with honest 404s; disbandClan ran DELETE FROM clan_chat, a table that never existed (live DB has clan_chat_messages), crashing every disband mid-transaction after members were already cleared. Dead false-advertising UI removed: unmounted admin/vip page (465 lines incl. a fake-success cancel toast), unreachable game-page BATTLE_LOG/INVENTORY Coming Soon views (zero setCurrentView callers; live equivalents exist), game:request_tile_info socket stub (no emitter). Doc truth: placeholder-auth claims on real-auth routes rewritten (chat route header/notes, chat/delete docblock, chat/edit double docblock), MessageThread's commented real-time sketch (wiring shipped in FID-004), ChatPanel's ask-veterans shipped TODO (FID-005), help page's "PvP combat is coming soon" (PvP has been live since the protection arc). FID candidates recorded in §5, not executed: auction clanOnly silent API-only lie (product call), channel mark-as-read, chat channel-ban wiring, moderator undelete + message:deleted fan-out, BattleLog type dup. Pins: 6 route behavior (gate-before-delete, false-success regression class) + disband table truth (sql-chunk walk + committed-migration snapshot). Gates: suite 1224/1224, tsc 0, eslint clean. | 2026-09-19 (this session) | Closed (2026-09-19: closed on 1ca470a; archived; CHANGELOG 0.0.25) |
| 110 | **FID-20260919-013 — real player-notification delivery (the dead stack's job, done for real):** design-first grounding overturned the survey's shape — wmd:* pushes were already partially live (launch/impact/spy/votes fed the mounted WMDHub toasts), while the true gaps were research-complete (zero-reader wmd_notifications row, orphaned emitter) and auction outcomes (inbox rows written directly, bypassing messagingService, so FID-004 pushes never fired). Shipped lib/playerNotification as the single seam: persist to the System DM conversation (metadata systemType, unread jsonb bump) then push FID-004's exact wire payloads (message:receive / conversation:updated via the production mappers, now exported) plus a new typed notification:push event to the user:<username> room; 60s in-process dedupe for sweep reprocessing; DB-only degradation when getIO() is null (Vercel jobs); inboxless guards inherited. Producers rewired: all six auction event types, missile interception (target + launcher — previously audit-row only), missile impact (target), research completion (seam + the finally-called broadcastResearchComplete). 7 pins; live probe 12/12 (register → list → buyout → inbox row + unread bump + all three socket events captured on a live seller socket + dedupe drop). Gates: suite 1231/1231, tsc 0, eslint clean. | 2026-09-19 (this session) | Closed (2026-09-19: closed on 746b920; archived; CHANGELOG 0.0.26) |
| 111 | **FID-20260919-014 — Law 17 (schema-consumer pointer or removal ticket) + pre-push Gate 4:** outlaws the schema-truth drift class (wmd_alerts never written, wmd_notifications never read, phantom clan_chat). Census script classifies every exported pgTable by live writer/reader pairs outside its defining file; tickets recognized from dev/fids content; fail-closed (synthetic ghost exits 1). Protocol amended (echo doc + extended_laws: 17). First run caught four previously-unknown ghosts — chatReadStatus, shrineBlessings, wmdConfig (no consumers), wmdSuspiciousActivity (write-only via an unreachable writer) — ticketed in FID §4, follow-ups on row 112. Also repaired pre-existing corruption in the protocol summary table (truncated row 14, duplicated row 15). Gates: suite 1231/1231, tsc 0, eslint clean; census 63 tables — 50 live, 13 ticketed, 0 violations. | 2026-09-19 (this session) | Closed (2026-09-19: closed on 3ec3752; archived; CHANGELOG 0.0.27) |
| 112 | **Law 17 first-catch follow-ups — wire-or-remove the four ticketed tables** (chatReadStatus, shrineBlessings, wmdConfig: zero code references, tables exist in DB; wmdSuspiciousActivity: writer exists in wmdAdminService but zero callers, no reader): each needs a small FID deciding wire (e.g. chatReadStatus for channel mark-as-read) or remove (idempotent drop migration per FID-011 convention). Census currently passes on the §4 tickets; this row is the debt those tickets carry. **Resolved — all four wired (operator chose wire, no removals) via FID-20260919-015:** W1 chatReadStatus (migration 0036 unique pair, chatReadStatusService upsert + read-state, real PATCH /api/chat replacing the FID-012 no-op, GET /api/chat/read-state, ChatPanel badges now survive refresh); W2 shrineBlessings (shrineBlessingService grant ledger, non-fatal inserts in activate + boost-all, GET /api/shrine/blessings, ShrinePanel Recent Blessings; yield bonus stored as integer percent at the DB boundary); W3 wmdConfig (alertConfigService table-first fallback + upsert, enabled/minSeverity gate consulted by BOTH alert writers, GET/PUT /api/admin/wmd-config, AdminView display); W4 wmdSuspiciousActivity (flagExcessiveLaunches 24h/threshold-10/once-per-window, non-fatal trigger on the missiles POST path, admin reader, AdminView list — writer now has a live caller). Latent defect caught en route: flagSuspiciousActivity/createAdminAlert id formats overflowed varchar(24); both now use generateId(). 23 pins + 15/15 live probe. Gates: suite 1254/1254, tsc 0, eslint clean; census 63 tables — 54 live, 9 ticketed, 0 violations. | 2026-09-19 | Closed (2026-09-19: closed on 32c6f85; archived; CHANGELOG 0.0.28) |
| 113 | **FID-20260919-016 — consolidate the two WMD alert tables onto wmd_alerts:** grounding overturned the directive's premise. `wmd_alerts` was flagged as an orphan needing a writer, but it had **neither** a writer nor a reader — the admin health endpoint read `wmd_admin_alerts` all along (my earlier report was wrong). The two tables were near-twins for the same event class; `wmd_admin_alerts` was live (2 writers, 2 readers), while the richer `wmd_alerts` (incident references, channel model, ack/resolve lifecycle) had never been written to once. FID-20260919-011's removal migration had kept `wmd_alerts` on the false claim that admin health read it. Operator chose **consolidate**: both writers (`createAdminAlert`, `missileTracker.recordAdminAlert`) and both readers (admin health, `getWMDSystemStatus`) now target `wmd_alerts`; migration 0037 moves the 8 rows (`details → data`, `OPEN → ACTIVE`) and drops the twin; the health reader's "unacknowledged" is now ACTIVE (RESOLVED/ARCHIVED stop counting); `recordAdminAlert` populates the `missileId` reference column. 12 pins + 8/8 live driver (twin absent, migrated rows in-vocabulary, RESOLVED excluded, writer lands ACTIVE with `data`, migration re-run safe). Gates: suite 1262/1262, tsc 0, eslint clean; census 62 tables — 54 live, 8 ticketed, 0 violations. | 2026-09-19 | Closed (2026-09-19: closed on 21b2d34; archived; CHANGELOG 0.0.29) |
| 114 | **FID-20260919-017 — Law-17 ticket triage: 5 removes, 3 wires; the ticket queue is cleared:** triaged the 8 remaining ticketed tables (all 0 rows) with a live id-width probe and writer-reachability tracing (full evidence: `dev/LAW17-TICKET-TRIAGE-2026-09-19.md`). Operator approved 5 removes / 3 wires. **Removed** (migration 0038): `achievements` (read-only; the live store is `players.achievements` jsonb — the admin stats route now unnests that), `wmd_votes` (zero refs; superseded by `wmd_clan_votes`), `wmd_consequence_events` (writer behind a zero-caller function), `wmd_resource_pools` + `wmd_defense_grids` (reachable writers, no reader, no UI). **Wired:** three tables whose reachable writers were silently failing on varchar(24) PK overflow — successful spy missions could not complete, counter-intel sweeps recorded nothing, and a successful interception 500'd — now `generateId()` (23 chars), each with a reader mounted (intel Reports view in WMDIntelligencePanel; Interception Log in WMDDefensePanel). 12 pins + 11/11 live probe. Gates: suite 1274/1274, tsc 0, eslint clean; **census 57 tables — 57 live, 0 ticketed, 0 violations**. Adjacent finding recorded: the whole `clanConsequencesService` module is unwired (separate disposition). | 2026-09-19 | Closed (2026-09-19: closed on da29f5b; archived; CHANGELOG 0.0.30) |
| 115 | **FID-20260919-018 — wire the clan WMD consequence system (cooldowns + retaliation):** row 114's adjacent finding, dispositioned **wire** by the operator (not remove, and not as a bare hook-up). Missile launches were **consequence-free** — the module implementing the documented 24-72h post-attack cooldowns (`dev/architecture.md`) had zero callers, and was partially non-functional even if wired: `applyClanWMDCooldown` computed `cooldownUntil` then wrote `bankTreasuryMetal: sql\`${clans.bankTreasuryMetal}\`` (a self-assignment that never set `wmdCooldownUntil`); `isClanOnWMDCooldown` unconditionally returned `false`; the launch path consulted neither; retaliation rights had zero consumers; and the coded cooldowns (14–60 **days**) contradicted the design doc by ~10×. Operator-signed parameters: cooldowns **24-72h scaled by warhead** (TACTICAL 24h · STRATEGIC 36h · NEUTRON 48h · CLUSTER 60h · CLAN_BUSTER 72h), reputation penalty re-targeted from per-member `players.researchPoints` (the tech currency, which could go deeply negative) to the **clan research pool** with `GREATEST(0, …)`. Wired: cooldown writer now sets the real column + `lastWMDLaunch`; reader computes remaining time; **enforcement at launch** in `launchMissile` refusing a cooldown-clan launch **unless** the launcher holds a live retaliation right, which is then consumed (the missing consumer); `missileTracker` detonation branch calls `applyClanWMDConsequences` non-fatally; relations canonicalized (sorted pair); launcher notified via the FID-20260919-013 seam. **Migration 0039** converts the four compared timestamps to `timestamptz` — the live probe caught a 24h cooldown reading back as **28h** (the FID-20260916-009 D2 class 0032 fixed for `protection_until`); applied, all four verified converted. Also fixed a pre-existing lint hazard by rename rather than suppression: `useRetaliationRight` → `consumeRetaliationRight` (the `use*` prefix collides with React's hook namespace). **Honest correction:** an in-flight claim that `grantClanRetaliationRights` overflowed a `varchar(24)` PK was **wrong** — `wmd_retaliation_rights.id` is `varchar(50)` (probed live); no overflow existed here, so the `generateId()` normalization is convention only and both the code comment and probe label were corrected. **`clan_relations` and `wmd_retaliation_rights` now have real consumers** instead of hanging off dead code. 11 pins + **23/23** live probes — including the real `launchMissile` refusal, the retaliation bypass/consume, and an **integrated leg** driving a due missile through the tracker's own `processDueMissiles()` to prove the detonation hook fires in the live impact path (not just that the service works in isolation). Gates: suite 1285/1285, tsc 0, eslint clean; census 57 tables — 57 live, 0 ticketed, 0 violations. | 2026-09-19 | Closed (2026-09-19: closed on 6ee7e79; archived; CHANGELOG 0.0.31) |
| 116 | **FID-20260923-001 — timestamp timezone integrity (the naive-column class + the two open retaliation calls):** the Perfection Loop ran on the document (5 passes, converged) and all three blocking decisions were signed, then the plan was implemented on operator go-ahead. Migration `lib/db/migrations/0040_timestamptz_conversion.sql` converted **135 naive columns → `timestamptz`** (119 `America/New_York`, 16 `UTC`, 57 carrying data; guarded and idempotent — re-applying a `timestamptz AT TIME ZONE` cast would silently re-shift an already-converted column, so the block skips anything already `with time zone`). 134 schema declarations flipped to `{ withTimezone: true }` across 14 files via the idempotent codemod `scripts/codemodTimestamptz.mjs`; `scripts/classifyTimestampColumns.ts` gained `--emit-sql` so the migration is generated from the evidence rather than hand-transcribed, and its report is `dev/TIMESTAMP-CLASSIFICATION-2026-09-23.md` (117 APP-LOCAL / 6 MIXED / 2 DB-UTC / 10 NO-WRITER). `scripts/timestampConventionCensus.cjs` (fail-closed) wired into `.githooks/pre-push` as Gate 5. Both open retaliation calls settled as signed: window `30d → 7d` with the rationale in code, and the clan WMD cooldown + live retaliation rights surfaced via a new `getPlayerWmdStatus()` through the missiles GET into `WMDMissilePanel`. **Dev DB verified 165/165 timestamptz, 0 naive**; live instant-preservation driver **10/10** (app-local columns keep the exact instant the app computed, the DB-default column lands on its UTC-wall instant, the `timestamptz` control is untouched). Gates: suite 1291/1291, tsc 0, eslint 0, all census gates exit 0. | 2026-09-23 | Closed (2026-09-24: closed on `19a51e2` — canonical, the class elimination; the §5.6 signed retaliation decisions shipped in `0e4c468`; Gate 5 wired by `2e2091a`; FID archived to `dev/fids/archive/`; CHANGELOG 0.0.32). **Closure probe re-run fresh at `2e2091a` (Law 16, 2026-09-24): tsc 0 · suite 1301/1301 (135 files) · all four censuses exit 0**; each commit's tree was also gated on its own (`0e4c468` 1285/1285, `19a51e2` 1291/1291). Status field corrected to `verified` on 2026-09-23: it read `loop-complete` (*plan final, pending implementation*) while §7 already documented a shipped implementation, and the document's closing "No code has been written" line was false. Honest limitation recorded in §7: the `America/New_York` calibration is correct for the environment that wrote the data (this DB is a fixture); a DB whose naive rows were written by a UTC process needs `UTC` for those rows. |
| 117 | **FID-20260923-002 — host-timezone game-time math (the class that survived the timestamptz fix):** -001 fixed where instants are **stored**; this fixes where the app decides what a *game day / game hour* **is** — server code read the host process timezone, so the same schedule fired at a different wall clock per host and shifted twice a year across DST. New `lib/gameTime.ts` resolves every boundary in one explicit zone (`GAME_TIMEZONE = America/New_York`, chosen so current host behaviour is preserved exactly while other hosts now match it). Ten scheduled sites rewired — beer respawn **including the second, duplicated implementation** in `lib/wmd/jobs/beerBaseRespawner.ts`, the bot "Sunday 4 AM" check, the harvest reset boundary and countdown, the AM/PM raid-period guard, daily territory income, the clan-distribution reset, the chat month bucket, the economy day — and ten `setDate(getDate()-N)` retention/expiry walks normalised to exact-ms. `scripts/hostTimezoneCensus.cjs` (fail-closed, exit 2 on scan error, client display exempt) wired into `.githooks/pre-push` as Gate 6. **Suite host-independence closed:** running under `TZ=UTC` exposed 3 real failures, all host-local assertions in `lib/beerBaseService.test.ts`; the suite zone is now pinned in `vitest.setup.ts` (`TEST_TZ` override); two tests that *mirrored* production instead of testing it were fixed — the `baseRaidFidelity` hand-copied duplicate became `lib/raidPeriod.ts`, now imported by **both** route and test (its old comment claimed it would break on drift; it could not) — and `stripeVipKeying` gained an absolute DST-crossing case pinning exact-duration arithmetic. `npm run test:zones` runs the two-zone union audit via `scripts/zoneAudit.cjs`. Gates: suite **1301/1301** in UTC, America/New_York and Asia/Tokyo; tsc 0; eslint 0; all four censuses exit 0. | 2026-09-23 | Closed (2026-09-24: closed on `200f4c3` — canonical, the class fix; the suite host-independence closure shipped in `106af8c`; Gate 6 wired by `2e2091a`; FID archived to `dev/fids/archive/`; CHANGELOG 0.0.32). **Closure probe re-run fresh at `2e2091a` (Law 16, 2026-09-24): tsc 0 · suite 1301/1301 (135 files) · all four censuses exit 0**; each commit's tree was also gated on its own (`200f4c3` 1300/1300, `106af8c` 1301/1301, `2e2091a` 1301/1301). Status field corrected to `verified` on 2026-09-23: the old value `implemented (uncommitted)` is **not in the protocol's allowed status list**, and the legacy-synonym map sends `implemented → closed` only when G2-backed. **Not done, deliberately:** the residual `policy.adminLogDays` purge — the dead arithmetic was removed and the defect recorded, but implementing the purge is a data-retention product call for the operator. Second-order finding exposed by the refactor and recorded in the FID. |
| 118 | **FID-20260924-001 — FID status vocabulary: `implemented` and `no-action` added; archival moved to terminal statuses:** the status set had **no lawful value for *implementation complete, gates green, commit outstanding*** and **no lawful way to retire a terminal finding record with no code to commit**. Both gaps were hit for real — FID-20260923-001/-002 spent a day parked on `verified`, whose spec meaning is *partially-executed work*, each §8 recording the misfit, while the accurate word (`implemented`) was a **legacy synonym for `closed`**, i.e. it read as an unlawful closure claim without a hash — and the spec was internally inconsistent besides: its lifecycle diagram already named an `Implemented` stage the status list omitted. Operator decisions (2026-09-24): **add `implemented` as a first-class status** (over amending G1 so approved level-3 sessions commit their own work, and over keeping the gap); **add a no-action disposition and archive FID-20260917-005** (over a one-off `closed (no-action)` backed only by the SCOPE row, and over leaving it live). Shipped: `allowed_statuses` + `terminal_statuses: [closed, no-action]` in `protocol.config.yaml` (re-parsed with the project's own YAML reader to prove the file still loads), the spec's amendment banner + status definitions + archival rule + FSM note, and the template comment block every new FID is copied from. `closed` stays strictly G2-gated and `implemented` is explicitly **not** archival-eligible; the legacy `implemented → closed` synonym is scoped to pre-2026-09-24 archived history, since the word now means the opposite on live files. FID-20260917-005 retired as the first `no-action` FID, prior value preserved, row 78 named as the backing record. Filed, audited and closed in one session; the first document to *use* the status it creates. Gates: suite 1301/1301, tsc 0, four censuses exit 0 (documentation/config change — regression checks; the load-bearing proof is the three-surface consistency output in the FID §8). | 2026-09-24 | Closed (2026-09-24: closed on `43ab259`; FID archived; CHANGELOG 0.0.32) |
| 119 | **FID-20260924-002 — ledger-integrity census gate (pre-push Gate 7): the record of truth was the one artifact nothing checked:** five censuses already audited the *codebase*; none read `dev/fids/` or `SCOPE.md`, so every ledger assertion — a FID status, the archival boundary, a cited commit hash — was unchecked by construction. New `scripts/ledgerIntegrityCensus.cjs` fails-closed on three real classes: a live FID whose status is outside `allowed_statuses` **read from `protocol.config.yaml` at every run** (never copied into the script — the vocabulary was amended twice by hand, 2026-09-16 and 2026-09-24, and this is what stops a third drift), a TERMINAL FID (`closed`/`no-action`) still parked in `dev/fids/` (archival is what terminal *means*), and a SCOPE.md ledger line citing a commit that does not resolve — an unverifiable claim wearing the clothes of a verified one. Four such citations exist today (`af1e61e`, `23cdc63`, `53c1531`, `49b5991`) and **all four are destruction-by-design** (an unlawful 2026-09-03 checkpoint commit, the tip it was reset from, a superseded post-filter-branch remote tip, a gc-pruned stash base), so they are **waived BY REASON** in a dated table rather than silently re-pointed at a hash that happens to resolve — the citation is the last record of a state that no longer exists. Report-only lines keep the gate honest where a rule permits the shape: 37/175 archived FIDs carry non-terminal historical labels (preserved by rule), and 7 cited hashes resolve but are unreachable from HEAD (they live on `backup-main-pre-rewrite` / remote-tracking refs — failing on them would push toward deleting the refs that hold rewritten history). Fail-closed on missing `dev/fids/`, missing SCOPE.md (check 3 would otherwise be vacuous), unreadable config, malformed/empty vocabulary, unusable git, a partial `--batch-check` read or an empty `rev-list`. All 83 cited hashes resolve in **one** `git cat-file --batch-check` pass — **0.34s**, after the first per-hash version measured `real 0m8.464s` and was replaced, because 8s on every push is how a gate gets quietly unwired. 12 tests, including a guard that the passing cases assert `1 live FID(s)` so checks A/B cannot pass vacuously against an empty directory. **Verified by negative drill, not by assertion:** a temporary live FID with `Status: underway` made the real pre-push hook refuse (`HOOK EXIT=1`, `LIVE FID STATUS OUTSIDE allowed_statuses (1): dev/fids/FID-20990101-999-gate-drill.md:4 status \`underway\``), then was removed. Gates at `e28731b`: tsc 0 · suite **1313/1313** (136 files) · census exit 0 · full pre-push chain exit 0, all seven gates green. **Not done, recorded:** `npm run lint` is not clean repo-wide — 6 pre-existing errors in files this change does not touch (see row 120's sibling note in the FID §7), so 0.0.32's recorded "eslint 0" is stale for the repo as a whole. | 2026-09-24 | Closed (2026-09-24: closed on `e28731b`; FID archived; CHANGELOG/VERSION 0.0.33). Retro-filed and closed in one pass — the operator directed the gate mid-session, so the FID records the work (the FID-20260919-014 shape) rather than gating it; G2 is the implementation commit `e28731b`, which the FID's §7 probe was run against (`git diff --stat e28731b` → empty). |
| 120 | **Gate 3 (Mongo eradication) could never pass: it refused every push whose scan was CLEAN — `main` has been unable to reach the remote since it was added** (FID-20260924-002 §2 findings 4–7): found by this session's gate drill, not by review. The gate ran grep into a variable and treated **any** non-zero exit as an enumeration failure, refusing with exit 2 — but grep exits **1 for "no matches"**, which is precisely this gate's pass state, so the hook could only refuse: exit 2 on a clean tree, exit 1 on a dirty one. It was inverted, not merely noisy. The refusal's own diagnostic named the opposite of the cause (`grep exit 0` beside a refusal triggered by exit 1) because the block's first `echo` reset `$?` before the message was composed — a pass impersonating a tool failure, the mirror of the defect class hardened out of these hooks on 2026-09-16. Corroboration: `refs/remotes/origin/main` is still `25e8078` (2026-09-17) while Gate 3's own header dates it 2026-09-19. Fixed by capturing grep's status before any other command and refusing only on `>1`, with the exit-code contract written into the hook (0 = matches, 1 = clean, >1 = real failure). Evidence: the same hook run that ended `HOOK EXIT=2` with `mongo census enumeration failed (grep exit 0)` now prints `✅ pre-push: mongo eradication census clean` and continues to Gate 2, ending `HOOK EXIT=0` with all seven gates green. | 2026-09-24 | Fixed (2026-09-24, commit `e28731b`; backing record FID-20260924-002 §2 findings 4–7 and §7). Indexed as its own row so a push-blocking defect is findable without reading the gate's FID: a gate that cannot pass is a worse failure than the drift it was written to catch. |
| 121 | **FID-20260924-003 — the lint baseline is cleared and gated (pre-push Gate 8): six eslint errors sat on `main` while the chain reported that the gates passed:** the sharper finding is not the six errors but the **omission** — `protocol.config.yaml` declares three Law-3 verification commands (`npx tsc --noEmit`, `npm run lint`, `npm run test:ci`) and the pre-push chain ran **none** of them (pre-fix grep of the hook for `npm run` / `eslint` / `vitest` / `tsc` matches only Gate 3's file-list arguments), so a green chain meant "the gates that exist pass" and the CHANGELOG's `eslint clean` (18 occurrences, `eslint 0` 16 more) read as tree-wide verification that nothing performed. All six are leftovers from real refactors rather than typos — provenance by `git log -S`: `HarvestRecord` by `c9c3189` (2026-09-19, lib batch off the shim), `countFlags` by `fb5b702` (2026-09-18, anti-cheat on drizzle/pg), `type CatalogEntry` by `ba16ab4` (2026-09-19, market/chat catalog), the apostrophe by `08410d6` (2026-09-19, tradeable listings), `sql` in `spawnBots` by `30f694e` (2026-09-19, spawnBots off the shim), `sql` in the notification driver by `746b920` (2026-09-20) — five of six inside the very release cycle whose entries claim `eslint clean`. Cleared by **removing the dead thing or escaping it, never by `eslint-disable`**: two unused import specifiers, two unused `sql` bindings, one `&apos;` escape matching house style, and one **finding** — `scripts/e2eSlice1AntiCheatLive.ts` defined an anti-cheat `countFlags` helper that nothing ever called, deleted rather than wired into a new assertion (inventing a probe check as a side effect of a lint fix would widen a live driver under cover of a tidy-up). **Gate 8** runs the project's own `npm run lint` **repo-wide** — a changed-files-only variant was rejected because the drift to catch is exactly the error already in the tree — refuses warnings as well as errors (the contract is 0/0) and refuses with exit 2 if `npm` is not on PATH rather than reporting a chain it never ran. **Verified by drill:** a throwaway file with one unused binding made the real hook refuse (`HOOK EXIT=1`, `'unusedByDesign' is assigned a value but never used`), then the chain went green at 8 gates. Gates at `ef163af`: **lint exit 0 with no output at all** (0 problems, so not even a summary line — repo-wide clean for the first time in the recorded ledger) · tsc 0 · suite **1313/1313** (136 files) · chain exit 0, 8 green gates. **Still open, recorded and NOT implied fixed:** the chain runs lint **only** of the three Law-3 commands — typecheck (~10s) and `npm run test:ci` (~25s) remain unwired, so no entry may read as if "the gates pass" covers them. | 2026-09-24 | Closed (2026-09-24: closed on `d3ad827` + `ef163af`; FID archived; CHANGELOG/VERSION 0.0.34). Two commits, fixes first: a chain commit whose own tree would fail its new gate is a worse artifact than either half alone. |
| 122 | **FID-20260924-004 — all three Law-3 verification commands are now gated, and CI runs the identical chain:** closes the open item FID-20260924-003 recorded rather than hid. `protocol.config.yaml` declares three verification commands and the chain ran one of them (`npm run lint`, Gate 8); **Gates 9 (`npx tsc --noEmit`) and 10 (`npm run test:ci`) join it**, run unchanged and placed after lint so the cheap gates fail first. **The decision, which outlives the change: the chain is ONE definition with TWO callers.** New `.github/workflows/gate-chain.yml` does not re-list the gates — it pipes a synthesized ref line into `.githooks/pre-push` itself, so what runs on a local push and what runs on GitHub cannot drift; the hook's header now documents its stdin contract as an interface. The repo had already stated the philosophy in `attribution-guard.yml` ("local hooks protect this clone; CI protects EVERY clone") and already knew the trap: `core.hooksPath` does not travel with a clone, which is why `scripts/ensure-hooks.js` exists — a local-only chain is not an enforcement boundary, and a CI-only chain leaves every local verification claim unchecked. Rejected with reasons: duplicating the gate list as CI steps (two definitions that disagree is the class Gate 7/8 exist to catch), a shared `scripts/gateChain.cjs` runner (same merit, rejected on scope — recorded as the next step if CI wants per-gate UI granularity), the suite in CI only (local feedback is the point: six lint errors survived a whole release cycle unseen), and folding in `npm run test:zones` (triples the suite; own decision). **CI can run DB-less, evidenced not assumed:** `vitest.setup.ts` stubs `DATABASE_URL` itself and every db-touching test mocks `@/lib/db` — `battleStatsService.test.ts` says "CI is DB-less" outright; `printenv DATABASE_URL` is unset on this host and the suite still passes 1313/1313. **Both new gates proven to refuse, on the real hook:** a lint-clean but type-invalid file passed Gate 8 and was refused by Gate 9 (`error TS2322`, chain exit 1); a lint-clean, type-clean failing test passed Gates 8-9 and was refused by Gate 10 (`AssertionError: expected 1 to be 2`, chain exit 1). Green chain afterwards: **10 gates, exit 0**; tsc 0; lint 0 errors / 0 warnings; suite 1313/1313. **Cost measured, because a slow gate nobody is told about is a gate nobody keeps:** full chain 74.5s / 74.6s / 74.8s across three runs at the current 126-commit range — lint 21.5s + suite 21.7s + tsc 3.6s + five censuses 2.1s + Gate 3 grep 0.2s, with Gate 2's attribution scan costing ~16s because it spawns `git log` *and* `grep` per commit (≈1s for a typical small push, so a normal chain lands near 50-60s). Capturing gate output was measured as free (lint 21.9s captured vs 21.9s direct), so the capture pattern is not the cost. `--no-verify` remains the escape hatch, survivable only because CI mirrors the chain. **Recorded limitations:** the workflow has never executed on GitHub (Actions cannot run from this machine — its YAML parses and its one chain-invocation line was run verbatim locally, exit 0); `node-version: lts/*` diverges from this host's Node 25.2.1 (no `engines`, no `.nvmrc`, and inventing a floor was rejected); Gate 2's per-commit cost is measured and left unfixed; the suite emits ANSI escapes even when not a TTY, which silently degraded the count in Gate 10's first draft to a fallback string (now stripped). | 2026-09-24 | Closed (2026-09-24: closed on `7ad976a`; FID archived; CHANGELOG/VERSION 0.0.35). Filed and closed in one pass on an operator directive that named the work. |
| 123 | **FID-20260925-001 — the session record had a six-day hole: the four 2026-09-24 gate closures shipped with no session summary, and nothing checks that one exists:** the campaign that made the chain trustworthy (nine commits, releases 0.0.33-0.0.35 — the ledger-integrity census as Gate 7, the lint baseline cleared and gated as Gate 8, typecheck and the suite gated as Gates 9-10 plus a CI caller of the identical chain) was recorded in commit messages, FIDs, SCOPE rows and the CHANGELOG, and in the session log nowhere. Measured, not estimated: a grep for FID-20260924 / ledger-integrity / Gate 8 / Gate 9 across `dev/session-summaries/` exits 1 with no hit; 71 commits landed on 2026-09-19..09-22 with no summary; no summary mentions any release between 0.0.13 and 0.0.31. Retro-filed as `dev/session-summaries/SESSION-2026-09-24-002.md` (commit `9b7125e`), dated to the day the work shipped, with the fresh 10-gate chain probe at `470e17b` pasted in full and the skipped intent log declared rather than implied. | 2026-09-25 | Closed (2026-09-25: closed on `9b7125e`; FID archived; CHANGELOG/VERSION 0.0.36). A record of a gap, not a plan that gated work — the FID documents a record that did not exist when the work happened. |
| 124 | **The session-summary requirement is enforced by nothing — and the hole is six days wide, not four units** (FID-20260925-001 §2 findings 3-4) — operator decision needed: no hook, workflow, script or census reads `dev/session-summaries/` (grep over `.githooks/*`, `.github/workflows/*.yml`, `scripts/*.cjs`, `scripts/*.mjs` → exit 1, no hit), so the README's required structure and the auto-archive rule's "log the archival in the session summary" are conventions that failed silently once already (71 commits on 2026-09-19..09-22; releases 0.0.13-0.0.31 with no summary of their own). Options: (a) make a session record required per closed-FID session date, by extending the ledger census or adding a corpus check — needs its own FID and drill, since FIDs and summaries cite each other in neither direction today; (b) check the `Last updated` header against `VERSION` — that field has now gone stale twice in six days; (c) retro-file the 2026-09-19..09-22 span; (d) start clean and state the convention change. | 2026-09-25 | Open — partially actioned. Option (a) was authorized by the operator's 2026-09-25 "resume" and implemented the same day as check D in `scripts/ledgerIntegrityCensus.cjs` (FID-20260925-002, commits `9a5ba11` + `2e1d63b`), with both failure modes drilled on the real hook. Options (b) check the `Last updated` header, (c) retro-file the 2026-09-19..09-22 span, (d) start clean and state the convention change remain open — each still changes a gate's contract or does work no directive has named. |
| 125 | **`SCOPE.md`'s own structure is unchecked** (found while grounding; FID-20260925-001 §2 finding 9): the `[OPEN-OUT-OF-SCOPE] — Discovered, Awaiting Operator Decision` heading appears twice — line 448 heads a block of 2026-09-08..09-15 session logs while the real item table starts at 1034 — so a reader looking for open items lands in the wrong section; the item table ends with an out-of-order row 30 after row 65 (line 1102); that single 4-column table carries both the open items (1-…) and the session ledger (…-122), so "which items are open" is not answerable mechanically; and the `[DEFERRED] / [OUT-OF-SCOPE] — Operator-Confirmed` section still reads "(none yet)". | 2026-09-25 | Open — no structural edit made; filed so the observation is a decision instead of an unnoticed defect. |
| 126 | **FID-20260925-002 — the session record is now a gate: ledger census check D (row 124 option (a), authorized by "resume"):** nothing read `dev/session-summaries/` (grep over `.githooks/*`, `.github/workflows/*.yml`, `scripts/*.cjs` → exit 1), so the README's Law-8 requirement and the auto-archive rule were conventions — and FID-20260925-001 is the proof that conventions fail. Check D adds three fatal rules to Gate 7 instead of an eleventh gate: (D1) a terminal FID closed on/after 2026-09-24 must be cited by a session summary; (D2) a terminal FID filed on/after that date must carry a dated closure, because an undated one cannot be judged and "unjudged" is how a check becomes decorative; (D3) a closure dated before the filing is impossible, which closes the backdating evasion. The cutover is derived rather than chosen — probing the archive gave 36 dated terminal closures, 8 closed on/after 2026-09-24, 8/8 cited — while an unscoped rule would have failed 84 of 179 FIDs, including the six recorded only as the range `012..017`. History stays advisory (134 terminal closures reported, never failed) so the 2026-09-19..09-22 question remains the operator's (row 124). Drilled on the real hook: uncited closure → `HOOK EXIT=1`, backdated closure → `EXIT=1`; gate cost 0.357s; suite 1320/1320. | 2026-09-25 | Closed (2026-09-25: closed on `9a5ba11` + `2e1d63b`; FID archived; CHANGELOG/VERSION 0.0.37). Two commits because the third rule — the backdating check — was designed after the first drill exposed the evasion it closes. |
