...
| 112 | **Law 17 first-catch follow-ups — wire-or-remove the four ticketed tables** (chatReadStatus, shrineBlessings, wmdConfig: zero code references, tables exist in DB; wmdSuspiciousActivity: writer exists in wmdAdminService but zero callers, no reader): each needs a small FID deciding wire (e.g. chatReadStatus for channel mark-as-read) or remove (idempotent drop migration per FID-011 convention). Census currently passes on the §4 tickets; this row is the debt those tickets carry. | 2026-09-19 | Open |
