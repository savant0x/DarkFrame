...
| 34 | **Mongo-era `ObjectId` lingers in domain types** — `types/clan.types.ts` imports `ObjectId` from `mongodb` (a dependency retained only for legacy type shapes) and 5 interfaces (`ClanActivity`, plus 4 neighbors) type `_id?: ObjectId` while the pg rows carry string/uuid ids. Services bridging rows to domain types hit compile mismatches (worked around locally in `clanActivityService` via `ClanActivityWithStringId`). Domain-wide `_id` migration is a types-level campaign | 2026-09-07 (session 005) | Types migration is cross-cutting; operator decision on sequencing |
| 35 | **JoinClanModal filter UI is decorative** — it sends `name`/`minLevel`/`maxLevel`/`minMembers`/`maxMembers`/`publicOnly` params, but `/api/clan/search` only reads `q`/`page`/`limit`. The filter panel renders and filters nothing server-side. Route extension is a functional/API change | 2026-09-07 (session 005) | API change needs operator approval |
