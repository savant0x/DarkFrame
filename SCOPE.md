...
| 31 | **FundDistributionPanel PERCENTAGE/DIRECT_GRANT requests don't match the distribute route contract** — route requires `percentageMap` (PERCENTAGE) / `grants` (DIRECT_GRANT) and ignores `recipients`; the panel sends only `recipients`, so both methods 400 at runtime today. Surfaced by typing the request body against the route. Fix is functional (request construction), not typing | 2026-09-07 (session 004) | Out of approved typing scope; functional change needs operator approval |
| 32 | **`components/clan/JoinClanModal.tsx` types its state `Clan[]` but feeds it the `/api/clan/search` DTO** (same defect class as JoinClanView, fixed this session): its `clan.members.length` (line 148) and `clan.settings.requiresApproval` (line 374) read fields the DTO doesn't carry — full-check gate crashes / always-'Public'. Also outside the WIP-touched file set | 2026-09-07 (session 004) | Outside the approved touched-file set |
| 33 | **`lib/clanActivityService.ts:36` declares `details?: Record<string, any>`** — pre-existing baseline lint finding in a file outside the touched set (sibling of the ClanActivityFeed `ActivityDetails` typing done this session) | 2026-09-07 (session 004) | Outside the approved touched-file set |
