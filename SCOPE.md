...
| 21 | BeerBase respawner job crashes on Postgres: `beerBaseService.ts` calls Mongo-API `Collection.countDocuments` (routed through the compat shim) — job dies on every boot tick. Same old-model pattern: Mongo habits under a Postgres coat. ~~Note: the shim's boolean-filter coercion (fixed 2026-09-03, session 016) removes the `is_bot = true` invalid-SQL failure mode; remaining `countDocuments` behavior needs re-testing.~~ | 2026-09-03 (SESSION-2026-09-03-003) | Open (out of scope; awaiting operator FID approval) |
