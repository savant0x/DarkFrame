...
| 3a | (persistence — early slice pulled forward) | — | **DONE** — heartbeat 401/200, typing 200, bot-migration 200, rp-economy 200, my-bids 200, auction create/bid/buyout 200 all live-verified (see below). |
| 3b | app/api/admin/analytics/{activity-trends,resource-trends}/route.ts, lib/messagingService.ts, lib/warfareConfigService.ts, lib/activityLogService.ts, app/api/cron/{player-snapshot,purge-old-data}/route.ts, app/api/messages/read/route.ts | ~+45 −18 | **DONE (IMPLEMENT, live-verified).** Analytics 500 root cause found during verification: drizzle re-binds a `sql` template separately in select/groupBy/orderBy, so `FLOOR(... / ${intervalMs})` produced `$1/$2` vs `$4/$5` — Postgres matches GROUP BY expressions syntactically → 42803. Fix: interval inlined via `sql.raw` (fixed map value, never user input) → byte-identical expressions; both trends 200 with real data (24h: 25 buckets/128 actions; 7d: top-gatherers populated). messagingService unread JSON_SET/JSON_UNQUOTE/JSON_EXTRACT → pg `jsonb_set(... ARRAY[key]::text[], to_jsonb(n))` — send increments recipient key, read resets own key (both DB-asserted). warfareConfigService `JSON_EXTRACT(config,'$.version')` (function does not exist on jsonb — live-proven) → `(config->>'version')::numeric` (numeric so 10 > 2; TEXT order would return 10 < 2). activityLogService `randomUUID()` (36 chars) into `id varchar(24)` → `generateId()` (23 chars); login/move writes post-fix persisted, zero rows > 24 chars. Cron fail-closed: player-snapshot + purge-old-data matched literal `Bearer undefined` when CRON_SECRET unset (fail-open); now `!cronSecret ||` rejects (401 live). **New finding fixed:** messages/read had session identity but NO participant gate — non-participant could zero any conversation's unread map; `markMessagesAsRead` now enforces membership via `getConversationForParticipant`, and the route maps service denial to 403 (previously success:false returned as 200). Exploit matrix: non-participant 403 + unread untouched, participant 200 + key reset. |
| 2 | 29 routes across app/api (move, harvest, research, factory/{build-unit,produce}, player/{build-unit,upgrade-unit}, tutorial×4, messages×3, chat, clan/level, admin/{vip/grant,vip/revoke,migrate-factory-slots}, cache/stats, logs/cleanup, flag/init, stripe/verify-session, assets/images POST) + lib/messagingService.ts | ~+330 −70 | **DONE (IMPLEMENT, live-verified).** Session-identity routes: body/query username·playerId·senderId dropped, actor from session. Admin routes: `requireAdmin` gate (fame=200, verifybuyer=403 across the matrix). Bug-fixes beyond the gate: `factory/build-unit` treated the JWT STRING as the username (fake auth); `logs/cleanup` read the phantom 'token' cookie + hardcoded `isAdmin=false` (route could never succeed); `cache/stats` reset had a TODO gate; `stripe/verify-session` granted VIP to checkout metadata with no session + now identity-binds buyer===session (403 mismatch); `clan/level` POST awarded XP for body playerId (XP forgery); chat's hardcoded TestUser placeholder replaced with session-backed `getChatPlayerContext` (fixes all 4 chat handlers at the seam); messages family: inbox/history/read all session-bound + `getConversationForParticipant` membership check on history reads (404 for non-participants). Census: 33 no-auth mutating routes → 3 (login/register/stripe-webhook per §8 decision 2; the 9 clan routes were already session-gated via requireClanMembership — census false positives). |
| 2e | (exploit probes) | — | **LIVE-VERIFIED (dev :51234):** 16 unauthenticated POST probes (move/harvest/research/messages×2/player×2/tutorial×3/clan-level/vip-grant/vip-revoke/migrate/flag-init/stripe-verify) → **all 401**; 2 unauthenticated GET probes → 401; admin matrix admin=200/player=403 on 5 routes; body-username-ignored proven on move (session fame moved despite body username NOTME); VIP grant/revoke round-trip 200; response hygiene probe CLEAN (no password/email/signupIp/stripeCustomerId on player/move/build-unit/tutorial); player sessions still 200 on session routes + heartbeat. Gates: tsc 0 · 341 tests · eslint net −2 (33 vs 35 pre-existing in touched files). |
| 4 | (dead-wire rebuilds + M3 de-mock) | ~+1200 −220 across 40 files | **DONE (IMPLEMENT, live-verified on dev :51234).** All 34 dead endpoints rebuilt/reshaped (census re-run: **0 remaining**). **Migrations:** 0010 (phantom tables: clan_chat_messages, clan_activities, clan_territories, clan_alliances + beer_base_config.type), 0011 (referrals.invalidated), 0012 (payment_transactions — phantom behind stripe/subscriptionService raw SQL), 0013 (clan_invitations), 0014 (mod_log.target_id varchar(20)→24 — war declarations target clan ids). **Service repairs in the direct call path:** moderationService 8× mysql2 `affectedRows` → RETURNING-length checks + 4 id-overflow sites (userId-Date.now() up to 34 chars → generateId()); referralService 3× JSON_ARRAY_APPEND → jsonb `||`; friendService 2× affectedRows + accept-path randomUUID id overflow; clanActivityService affectedRows; clanAllianceService full mysql2→pg conversion (JSON_CONTAINS → jsonb @> array-probe, insertId → RETURNING, `.rows` access, rowToAlliance JSON.parse-on-jsonb crash); clanChatService `.map`-on-execute + MessageType mismatch; **authMiddleware.requireClanMembership JSON_CONTAINS → @> (every clan route 500'd)**; clanService+clanBankService 9× `players.mongoId` lookups → `players.username` (mongoId is NULL on every live row — the §5.1 residual, fixed at the seam); clanWarfareService ClanWarfareRow fantasy-shape (clan.level.currentLevel on a snake_case row) → real flat columns + declareWar treasury debit/insert now atomic (db.transaction) after failed mod_log inserts silently ate 100k treasury (money-loss class) + mod_log id overflow fix + attacker attribution via details.attackerClanId; clan_invitations INSERT omitted the id PK entirely → generateId()+RETURNING, route returns invitationId. **Admin endpoints:** referrals list/flag/validate/invalidate (DB-asserted validated:1, invalidated:1, flagged:1), vip/subscriptions (real stripe fields + payment_transactions; no fabricated MRR), beer-bases/recalculate-predictions, user/permissions (session-derived), bot-scanner/tracked (players.isBot + botConfig defeat data; totalLoot served as zeros flagged as data gap — not fabricated), moderation blacklist GET un-stubbed (getBlacklist) + client reshaped onto the real contract (9 dead calls removed). **Clan endpoints:** bank/deposit+withdraw (arithmetic DB-asserted: 2M − 1.5M clan cost − 50k deposit + 20k withdraw = 470k player / 30k treasury), promote/demote/kick (role cycle DB-asserted), search, chat send/messages/delete (soft-delete), activities + activity feeds, wars list (both sides verified), alliances list/create/break, war/declare (level gate, cost debit, self-war guard), territory list/unclaim. **Friends:** mongo clientPromise removed from requests + [id]; full send→accept→list cycle verified. **M3 de-mock:** dummy-messages fixture removed from chat GET; real chat_messages round-trip proven. **Data notes:** owner account `fame` has rank=1 so give-resources rank≥5 gate rejects the owner (logged, operator decision pending); `sent` friend requests reuse fromUsername for the recipient per route-documented convention (misleading name, load-bearing for the panel — flagged). |
| 5 | (de-mock + @ts-nocheck + lint-zero) | — | pending (M3 chat de-mock delivered early inside Phase 4 above) |
