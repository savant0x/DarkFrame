...
> Close-out note (2026-09-15 audit): §7 implementation shipped + gated at the
> time (migration 0018, contract reads). The tutorial system was later rebuilt
> around the JSON tracking contract (nearest-cave resolution, harvest hooks —
> FID-20260909-025 and followers); `getCurrentActionCount`/`recordActionAttempt`
> no longer exist in code. Closed as implemented-then-superseded.
