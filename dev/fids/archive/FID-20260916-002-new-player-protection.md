...
- **Commit hash (G2 — required for `closed`):** `0d18a93` — `feat: new-player 72h protection window — registration write, infantry/factory enforcement, aggression void (FID-20260916-002)`

---

**Commit executed (G1):** `0d18a93` (2026-09-16) — 11 files, path-scoped (SCOPE.md excluded; its closure row rides this archival commit). Pre-commit ladder gate green after an independent hook fix (`21a59fd`): the gate's summary matcher could never match vitest 4's ANSI-colored output — its own suite passed 10/10 while every commit was refused.

**Archive:** moved to `dev/fids/archive/` at close per the protocol (closed FIDs must not remain in `dev/fids/`).

---

**Final status:** closed (implemented + gates green + live probes 7/7; G2 satisfied by commit `0d18a93`, 2026-09-16)
