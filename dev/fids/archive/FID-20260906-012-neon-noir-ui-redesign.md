...
> Close-out note (2026-09-15 audit): direction approved and fully executed —
> Phases 0/1 here plus Waves A/B/C under the FID-20260908-006 umbrella and
> FIDs 007–021 (all archived closed). No outstanding scope in this file.
...
- **Status: CONVERGED — direction + phases approved by operator; execution underway (checkpoint: P0+P1, then StatsPanel-only reskin for live review).**
...
## Execution Loop Records

### Phase 0 (stack migration) ✅ — commit 3af56c8
- React 18.3→19, Tailwind 3.4→4 (`@tailwindcss/postcss` + `postcss.config.cjs` replacing ESM `.mjs` — Next's find-config resolves CJS here), `@import "tailwindcss"` + `@config` bridge keeping tailwind.config.ts authoritative, HeroUI 3.2.4 installed (dormant until Phase 3), joyride 2.9.3→3.2.0 with the exFAT alias hack **removed** (dual exports confirmed), emoji-mart + react-hot-toast dead deps removed, TutorialOverlay migrated to the v3 Step/callback API.
- Self-caught correction: the first `postcss.config.js` (CJS) was ambiguous under Next's resolution while the dead server cached stale state; renamed to `.cjs` to pin the require path.
- Gates: tsc 0 across the React 19 codebase, eslint clean on touched files (2 pre-existing `catch (error: any)` in MessageThread fixed per the operator's no-`any` policy), 348/348 tests, login + game visual parity confirmed live on :3002.
- Pre-existing finding recorded for the dead-destination sweep: `/shop` has no page (nav prefetch 404) — not a P0 regression.

### Phase 1 (token layer + StatsPanel reskin) ✅
- `app/neon-noir.css`: nn-* OKLCH token namespace (surfaces, signals, text, function-driven glows, motion) + HUD component classes (panel with corner brackets, scanline headers, segmented STR/DEF meter, chips, wells, semantic buttons/links). Additive — legacy theme untouched for all other components. Imported from globals.css after the TW4 bridge.
- `components/StatsPanel.tsx` fully reskinned: per-panel semantic accents (cyan=player/military, violet=XP/clan/shrine, amber=resources, green=harvest), Orbitron tabular numerals, `xpProgress` type declared on `Player` (removes both `(player as any)` casts at the source), zero-division guards on the STR/DEF meter, logic/handlers preserved exactly.
- Loop caught: 4 test failures were exact-copy assertions on intentionally reworded UI ("Join/Create"→"Join / Create", research "250"→"250 RP"); tests updated to the approved copy, not silenced.
- Gates: tsc 0, eslint clean, 348/348 tests, live game drive: panel renders with brackets/meters/accents, Base (73,70) correct (FID-009 fix visible), harvest math intact.
- **Awaiting operator's live review of the reskinned panel before Phase 2 (game shell).**

### Type-honesty sweep (operator directive: `unknown` = same escape class as `any`) ✅
- Operator rule adopted: **no bare `any` AND no `unknown` where a real shape is knowable.** Validation boundaries (zod input, untrusted API payloads pre-parse) keep `unknown` legitimately; everywhere else, types describe the data.
- **Root fix — `lib/playerService.ts` `mapRowToPlayer`:** the old `as unknown as Player` silently widened the DB row into the domain contract. Replaced with explicit construction: nullable columns normalized to optional fields via `orUndefined`, pg smallints → booleans via `bit()`, pg numerics → numbers, `referralRewardsEarned` composed from denormalized columns. Every one of ~60 fields is now compiler-verified against the contract.
- **Schema `$type`s fixed at the boundary** (`lib/db/schema/players.ts`, `tiles.ts`): `discoveries`/`achievements`/`stats` were typed `Record<string, unknown>` against real domain types (`Discovery[]`, `AchievementRecord[]`, `PlayerStats`); `lastHarvestedBy` was `$type<any[]>()` → `HarvestRecord[]` (new type).
- **Honest unions where columns truly hold two shapes:** `PlayerInventory.items: Array<InventoryItem | TutorialInventoryItem>` and new `AchievementRecord = Achievement | AchievementMarker`; shared narrowing predicates in **`lib/inventoryUtils.ts`** (Law 13 home), replacing annotation-cast filtering.
- **`SanitizedPlayer` is now THE client-facing contract** (types/game.types.ts): `MoveResponse.player`, GameContext state, clan views typed with it. Compiler now enforces "no private field ever reaches a client shape."
- **Getter overloads** in playerService: `{ includePrivate: true }` → `Player | null`, default → `SanitizedPlayer | null` (private impl pattern for the wrapper).
- **Cast-kills:** move route's `player as unknown as Record<string, unknown>` bridge + runtime re-validation chain deleted (mapper guarantees the shape — dead code); `direction as MovementDirection` replaced by `z.nativeEnum(MovementDirection)` in MoveSchema (drift impossible); extend route's `ITEM_VALUES as unknown as Record<string, number>` replaced by direct typed access.
- **Latent bug the typing exposed:** AutoFarm's unit comparator sorted on `b.str` (field doesn't exist on PlayerUnit → always 0) — "strongest units" selection never worked. Fixed to `strength` and real `CombatUnit`/`HarvestAttemptResult`/`CombatAttemptResult` types added to autoFarm.types.
- **ChatPanel wire contracts** (`ChatMessageWire`, `TypingWire`, `ConversationWire`, `DirectMessageWire`) replace five `usePolling<any>` + `map((m: any))` sites; all five `catch (error: any)` → bare catches with one `getErrorMessage` narrowing helper; MessageThread `catch (error: unknown)` → bare catches.
- Live-probe data verified against every wire type (server field `message`, `isVIP`/`senderIsVIP` dual alias documented at the boundary).
- Gates: tsc 0, eslint net-negative on the touched set (multiple `any`s removed, zero introduced; tutorialService's 13 `any`s are pre-existing backlog noted for its own FID), 348/348 tests.
