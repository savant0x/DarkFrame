# Deep Research Prompt: Unit Building System Redesign for DarkFrame
...
## Factory Context (Affects Unit Design)

Players build units at factories. Key factory properties:
- 2,250 factories on map (10% of 22,500 tiles)
- Factory slots determine how many units can be built per capture: `maxSlots = 5,000 + (level - 1) × 500`
- L1 factory: 5,000 slots | L5: 7,000 slots | L10: 9,500 slots
- Players cycle through factories: capture → fill slots → abandon → repeat
- Factory levels persist on the map when abandoned
- Over time, the map accumulates higher-level factories, allowing players to build larger armies per cycle
- Early game: small armies from L1 factories | Late game: massive armies from L10 factories

## Current Unit System — Complete Data

### Unit Count: 65 Types Across 5 Tiers + Specialized + Prestige
...
We need a complete unit building system redesign. The current system has these observable properties:
- 65 unit types across 5 tiers plus specialized and prestige
- Every STR unit has a mirror DEF unit with identical stats and costs
- Cost-per-STR is constant across all tiers
- PvP combat is purely "who has more STR"
- Two production systems exist (one is broken)
- Units have no special abilities, roles, or counters
- Players build units at factories by cycling through them (capture → build → abandon → repeat)
- Factory slots limit how many units can be built per capture (5,000-9,500 depending on level)

We do NOT know what the right answers are. We need deep research to determine:
- How many unit types there should be
- What roles units should have
- How unit costs should scale across tiers
- How PvP combat can involve tactical depth without real-time gameplay
- How the production system should work (one system, not two)
- How unit veterancy/experience could work
- What prestige/achievement units should offer
- How the slot/capacity system should interact with unit types
...
## Desired Output
A complete unit system specification with specific formulas, numbers, and tables. The research should determine the best approach for making unit building and PvP combat strategically interesting in a text-based automated-combat browser MMO where players cycle through factories to build armies.
