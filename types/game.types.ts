...
export interface Player {
  ...
  lastActive?: Date; // Most recent activity timestamp (activity tracking metrics)
...
export interface PlayerInventory {
  ...
  used?: number; // Current inventory usage (maintained by inventory route for legacy records)
  resources?: InventoryItem[]; // Stackable resource items (maintained by inventory route)
  gatheringBonus?: GatheringBonus; // Permanent digger bonuses (ensured by inventory route for legacy records)
  activeBoosts?: ActiveBoosts; // Deprecated temporary boosts (ensured by inventory route for legacy records)
}
 ...
 */
/**
 * Item granted by the tutorial system. Distinct from InventoryItem (a found/harvested
 * item with Position and rarity) — the players.inventoryItems column holds both shapes.
 */
export interface TutorialInventoryItem {
  ...
  quantity?: number;
  acquiredAt: Date;
  source: 'tutorial' | 'tutorial_completion';
}
