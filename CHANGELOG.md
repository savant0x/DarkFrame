...
## [0.0.37] — 2026-09-25 session

### Added — FID-20260925-002: ledger census check D — the session record is now a precondition of a closure, not a convention (closed on `9a5ba11` + `2e1d63b`)

- **The finding is one line of absence.** `dev/session-summaries/README.md` opens with Law 8 (*“the intended change is documented before implementation starts”*) and the protocol's auto-archive rule ends with *“Log the archival in the session summary”* — and `grep -n session-summaries` over `.githooks/*`, `.github/workflows/*.yml`, `scripts/*.cjs` and `scripts/*.mjs` exited **1**. Nothing read the directory. `FID-20260925-001` is the evidence that conventions alone fail: nine commits and four FID closures (releases 0.0.33-0.0.35) shipped with the chain green and the record empty.
- **A fourth check in Gate 7, not an eleventh gate.** The ledger census is the ledger's integrity surface; splitting “the ledger is intact” across two gates would fragment one meaning into two failure vocabularies. Three fatal rules: **(D1)** a terminal FID whose stated closure date is on/after **2026-09-24** must be cited by at least one session summary; **(D2)** a terminal FID *filed* on/after that date must carry a dated closure — an undated one cannot be located relative to the cutover, and “unjudged” is how a check becomes decorative; **(D3)** a closure dated before the FID was filed is impossible, which closes the obvious evasion (write an older date and D1 stops applying). Two refusals (missing or empty `dev/session-summaries/`) keep the check from passing vacuously on a tree with no record at all.
- **The cutover is derived, not chosen.** Probing the archive before writing the rule: `terminal WITH date: 36 · terminal WITHOUT date: 106 · closure date >= 2026-09-24: 8`, and **8/8 cited** (`FID-20260925-001`, `FID-20260917-005`, `FID-20260923-001/-002`, `FID-20260924-001..-004`). 2026-09-24 is the first day with a demonstrably complete record; a later cutover would exempt closures that *are* recorded, an earlier one would fail history the gate has no business rewriting.
- **Why the check is date-scoped — measured, not assumed:** an unscoped “every archived FID must be cited” rule would fail **84 of 179** FIDs (the 2026-09-06 generation predates the directory's use as a per-session log), and a range mention is not an ID — `FID-20260919-013` … `-017` are recorded (commit `3d75409`, rows in this ledger) but cited as `012..017`, so an ID-substring rule without date scoping would fail six FIDs that are properly recorded. The alternative — an 84-entry exemption list — would be noise, not judgement. History is therefore **reported, never failed**: 134 terminal closures predate the cutover or carry no date, and the 2026-09-19 … 09-22 question stays the operator's (row 124 options b/c/d, still open).
- **Proven by drill on the real hook, twice, because a check never seen to refuse is not a check:** an uncited closure (`dev/fids/archive/FID-20990101-999-gate-drill.md`, `closed (2099-01-01, …)`) → `FIDs CLOSED ON/AFTER 2026-09-24 WITH NO SESSION RECORD (1)` and `HOOK EXIT=1`; a backdated closure (`FID-20990101-998-drill.md`, `closed (2026-01-01, …)`) → `CLOSURES DATED BEFORE THE FID WAS FILED (1)` and `EXIT=1`. Both artifacts removed in the same command, census green afterwards. A third failure mode (undated post-cutover closure) is drilled by fixture; the file's tests go 12 → 19.
- **Cost stays negligible, which is the only reason a new check can be wired at all:** `0.357s` against `0.339s` before it (one directory listing and one read per summary) — the same standard `FID-20260924-002` set when it took the same script from 8.5s to 0.34s.
- Gates at `9a5ba11`: `CHAIN EXIT=0`, 10 green gates, suite **1319/1319**; at `2e1d63b`: `CHAIN EXIT=0`, suite **1320/1320**. This FID is itself subject to the rule it adds — `dev/session-summaries/SESSION-2026-09-25-001.md` cites it, and the census confirms `8 terminal FID(s) closed on/after 2026-09-24 cited by 113 session summary file(s)`.

- **Correction carried into the record (recorded, not buried):** this session's earlier `CHAIN EXIT=0` assertions were captured with `${PIPESTATUS[0]}` after `printf … | bash .githooks/pre-push … | tail` — which is **`printf`'s** status, not the hook's. The green verdict was independently corroborated (the successful run's last line is `✅ pre-push: attribution scan clean`, printed only on success), but the exit-code claim was measuring its own input. Re-measured from the hook directly: green → `CHAIN EXIT=0`, drills → `HOOK EXIT=1`. A dated addendum was appended to `SESSION-2026-09-24-002.md`, whose §4.2 carried the same string; the lesson is the project's own — a check that looks like a verdict must be probed before it is trusted.

## [0.0.36] — 2026-09-25 session

### Added — FID-20260925-001: the session record is retro-filled for the four 2026-09-24 gate closures, and the six-day hole it exposed is filed (closed, commit `9b7125e`)

- **The campaign that made the chain trustworthy was the one with no session record.** Four FID closures on 2026-09-24 (nine commits, releases 0.0.33-0.0.35: the ledger-integrity census as Gate 7, the lint baseline cleared and gated as Gate 8, typecheck and the suite gated as Gates 9-10 plus a CI caller of the identical chain) were recorded in commit messages, FIDs, SCOPE rows and this changelog — and in `dev/session-summaries/` nowhere. The open question is what that record was for: `README.md` opens with Law 8 (*“the intended change is documented before implementation starts”*) and the protocol's FID auto-archive rule ends with *“Log the archival in the session summary”*.
- **Measured, not estimated.** A grep for `FID-20260924` / `ledger-integrity` / `Gate 8` / `Gate 9` across `dev/session-summaries/` exits **1 with no hit**; **71 commits** landed on 2026-09-19 … 2026-09-22 with no summary; and **no summary mentions any release between 0.0.13 and 0.0.31** (the version strings simply stop at 0.0.12 until 0.0.31/0.0.32 appear inside `SESSION-2026-09-24-001`). The directive named four units; the hole is six days wide.
- **`SESSION-2026-09-24-002.md` is a retro-file and says so.** Dated to the day the work shipped (the directory's one-file-per-calendar-day grain) with the retro-filing date on the header line, §2 declaring that no intent was logged before implementation for any of the four units, the four operator directives quoted verbatim from each FID's `Trigger`, the plan steps, the recorded probe numbers per FID, and **a fresh probe at session end pasted in full** — the chain run over the real push range (128 commits) with all ten gates green in 75.4s, `tsc` 0, `eslint` 0/0, `1313 passed (1313)`, five censuses exit 0.
- **The structural finding, which is why this is a FID and not a chore: nothing can tell whether a summary exists.** No hook, workflow, script or census reads `dev/session-summaries/` (`grep` over `.githooks/*`, `.github/workflows/*.yml`, `scripts/*.cjs`, `scripts/*.mjs` → exit 1), so the requirement is a convention — and a convention failed silently while the chain printed ten green gates at the same tree. Recorded as **SCOPE row 124** with four options (require a session record per closed-FID session date; check the `Last updated` header against `VERSION`; retro-file 2026-09-19…09-22; start clean), left to the operator: each option either changes a gate's contract or does work the directive did not name.
- **A hand-maintained field has now gone stale twice in six days.** `SCOPE.md:9` read *“rows 116-117 dispositioned; CHANGELOG 0.0.32”* while rows 118-122 existed and `VERSION` stood at 0.0.35 — the same field `SESSION-2026-09-24-001` §3.2 had already corrected once. Corrected again and reported as a finding rather than only fixed: a claim about the ledger that nothing reads is the shape this project has twice hardened out of its gates.
- **Also filed (SCOPE row 125):** `SCOPE.md` has two identically-titled `[OPEN-OUT-OF-SCOPE] — Discovered, Awaiting Operator Decision` headings (`:448` heads a block of 09-08…09-15 session logs; the real item table starts at `:1034`), its table ends with an out-of-order row 30 after row 65 (`:1102`), and that one 4-column table carries both the open items and the session ledger, so “which items are open” is not answerable mechanically. No structural edit made — the observation is filed so it is a decision instead of drift.
- Ledger at the record commit: `0 live FID(s), 179 archived; 409 ledger line(s) … 90 cited hash(es)` → census exit 0 (two unchanged advisories: 37/179 archived FIDs carry historical non-terminal labels; 7 cited hashes resolve but are unreachable from HEAD).
- **Honest limitation, and it is the point of the FID:** writing the summary closes the gap for those four units and **cannot** satisfy Law 8 for them — the intent-before-coding step was skipped, and the file says so in its own §2. It also does not cover the 71 commits of 2026-09-19 … 2026-09-22, which remain unrecorded pending the row-124 decision.

### Recorded — `main` is 129 commits ahead of `origin/main` (nothing pushed)

- `refs/remotes/origin/main` is still `25e8078` (2026-09-17). The Gate 3 repair in 0.0.33 removed the push-blocking defect, so `main` *can* reach the remote again — publishing 0.0.32 through **0.0.36** is available and unexecuted, and remains an operator decision.
- Measured rather than asserted, since each local commit moves it: `git rev-list --count origin/main..HEAD` → **128** at `470e17b`, **129** at the record commit `9b7125e`, and one higher again once this ledger commit lands. The number belongs to a commit, not to the session — the same reason `SESSION-2026-09-24-001` declined to write its own hash into itself.

## [0.0.35] — 2026-09-24 session

### Added — all three Law-3 verification commands are gated, and CI runs the identical chain (FID-20260924-004, closed on `7ad976a`)

- **Gate 8 installed lint and left the other two unwired**, so the chain's coverage stayed a half-truth: green meant “the gates that exist pass”. `protocol.config.yaml` declares three verification commands; **Gate 9 (`npx tsc --noEmit`) and Gate 10 (`npm run test:ci`)** join the chain now, run unchanged and placed after lint so a typo does not cost a full suite run first.
- **The decision, which is the part that outlives the change: the chain is ONE definition with two callers.** New `.github/workflows/gate-chain.yml` does not re-list the gates — it pipes a synthesized ref line into `.githooks/pre-push` itself, so what runs on a local push and what runs on GitHub cannot drift apart. The repo had already stated the philosophy in `attribution-guard.yml` (“local hooks protect this clone; CI protects EVERY clone”) and already knew the trap the other way round: `core.hooksPath` does not travel with a clone — the reason `scripts/ensure-hooks.js` exists — so a local-only chain is not an enforcement boundary, and a CI-only chain leaves every local verification claim unchecked. The hook's stdin contract is now documented as an **interface**, because CI depends on it.
- **Rejected, with reasons:** duplicating the gate list as explicit CI steps (two definitions that disagree is the class Gate 7/8 exist to catch); a shared `scripts/gateChain.cjs` runner (same merit, rejected on scope — recorded as the natural next step if CI wants per-gate step granularity); the suite in CI only (local feedback is the point — six lint errors survived a whole release cycle unseen); folding in `npm run test:zones` (triples the suite; its own decision).
- **CI can run this DB-less — evidenced in-tree, not assumed:** `vitest.setup.ts` stubs `DATABASE_URL` itself (`process.env.DATABASE_URL ??= …`) and every db-touching test mocks `@/lib/db`; `__tests__/lib/battleStatsService.test.ts` says it outright (“CI is DB-less: db.execute is scripted per case”). `printenv DATABASE_URL` is unset on this host and the suite still passes 1313/1313.
- **Both new gates proven to refuse, on the real hook:** a lint-clean but type-invalid file passed Gate 8 and was refused by Gate 9 (`error TS2322: Type 'string' is not assignable to type 'number'`, chain exit 1); a lint-clean, type-clean failing test passed Gates 8–9 and was refused by Gate 10 (`AssertionError: expected 1 to be 2`, chain exit 1). Green chain afterwards: **10 gates, exit 0**; `tsc` 0; `lint` 0 errors / 0 warnings; suite **1313/1313**.
- **Cost measured, because a slow gate nobody is told about is a gate nobody keeps:** the full chain is **74.5s / 74.6s / 74.8s** across three runs at the current 126-commit range — lint 21.5s + suite 21.7s + tsc 3.6s + five censuses 2.1s + Gate 3's grep 0.2s, with Gate 2's attribution scan at ~16s because it spawns `git log` *and* `grep` per commit (≈1s for a typical small push, so a normal chain lands near 50–60s). Output capture was measured as free (lint 21.9s captured vs 21.9s direct), so the capture pattern is not the cost. `--no-verify` remains the escape hatch — survivable only because CI mirrors the chain.
- **Recorded limitations:** the workflow has never executed on GitHub (Actions cannot run from this machine; its YAML parses and its single chain-invocation line was run verbatim locally, exit 0 with 10 gates); `node-version: lts/*` diverges from this host's Node 25.2.1 (`package.json` declares no `engines`, there is no `.nvmrc`, and inventing a floor was rejected — a mismatch fails loudly on the first run); Gate 2's per-commit cost is measured and deliberately left unfixed; and the suite emits ANSI escapes even when not a TTY, which silently degraded Gate 10's count in its first draft to a fallback string (now stripped before matching).
...
- **Five censuses audited the codebase; nothing audited the ledger.** Inverted-route, Mongo eradication, schema-consumer (Law 17), timestamp convention and host timezone all inspect `lib/`, `app/api/` and the schema. FID statuses, the archival boundary and SCOPE.md's commit citations are all *assertions about what happened*, and no check had ever compared one against the rule that defines it — so the vocabulary drifted twice (2026-09-16 `converged` → `loop-complete`; 2026-09-24 `implemented` + `no-action`) and each time it was found by hand, days later.
- **Gate 7 fails on three classes, all of them real today.** (1) A live FID whose status is outside `allowed_statuses` — read from `protocol.config.yaml` **at every run**, never duplicated into the script, so an amendment to the vocabulary and the check enforcing it cannot drift apart (a missing or empty key refuses rather than passing). (2) A **terminal** FID (`closed` | `no-action`) still parked in `dev/fids/` — archival is what terminal *means*, and a terminal file left live is how “how many FIDs are open” stopped being answerable from the directory. (3) A SCOPE.md ledger line citing a commit that does not resolve: a citation is evidence, and an unresolvable hash is an unverifiable claim wearing the clothes of a verified one.
- **Four cited hashes genuinely do not exist — and all four are destruction-by-design, so they are waived BY REASON rather than re-pointed.** `af1e61e` (the 2026-09-03 checkpoint commit made without operator approval, removed by decision and never pushed), `23cdc63` (the tip it was reset from), `53c1531` (the remote tip after the filter-branch force-push, superseded by the later history import), `49b5991` (base commit of the retained May-era stash — the stash was dropped and the object gc-pruned). Each citation is the last record of a state that no longer exists, so re-pointing it at a hash that happens to resolve would falsify the audit trail. The allowlist sits in the script with the reason and the probe date, and a **new** dead hash is a failure, not a new entry.
- **Two report-only lines keep the gate honest where a rule permits the shape.** 37 of 175 archived FIDs carry non-terminal historical labels (`loop-complete`, `verified`, `implemented`, one `completed`/`complete`/`created`/`fixed` each, and 12 pre-template-era files with no parsable `Status` at all) — archived labels are preserved by rule, so this is reported, never fatal. 7 cited hashes resolve but are not reachable from HEAD (`2426cf4 f7f0921 049459b` on remote-tracking branch tips; `4674b73 0e82eb5 8be0bde de914fa` on `backup-main-pre-rewrite`) — failing on those would push toward deleting the refs that still hold rewritten history.
- **Fail-closed everywhere:** missing `dev/fids/`, missing SCOPE.md (check 3 would otherwise be vacuous), unreadable config, malformed or empty vocabulary, unusable git, a partial `--batch-check` read, or an empty `rev-list` all exit 2 — never 0. Same rule as the 2026-09-16 hardening: a tool failure must not impersonate a pass.
- **0.34s, because a slow gate gets quietly unwired.** The first implementation resolved each of the 83 cited hashes with its own `git cat-file -e` subprocess and measured `real 0m8.464s` on this host; it now resolves all of them in **one** `git cat-file --batch-check` pass (one answer line per request, asserted against the request count so a short read refuses) at `real 0m0.339s`. Recorded because 8s on every push is how a “temporary” gate gets deleted.
- **Verified by negative drill, not by assertion.** A gate that has never been seen to refuse is not evidence of a gate: a temporary live FID with `Status: underway` made the real pre-push hook refuse (`HOOK EXIT=1`, `LIVE FID STATUS OUTSIDE allowed_statuses (1): dev/fids/FID-20990101-999-gate-drill.md:4 status \`underway\``), then the file was removed and the chain went green. 12 tests cover the real tree plus every failure mode (bogus status, archive-scoped synonym used live, terminal FID parked, unparsable `Status`, unresolvable hash, fenced-block immunity, no-live-dir and missing-SCOPE refusal), and the passing cases assert `1 live FID(s)` so checks A/B cannot pass vacuously against an empty directory.
- Gates at `e28731b`: tsc 0 · suite **1313/1313** (136 files; 1301 + 12 new) · census exit 0 · full pre-push chain exit 0 with all seven gates green.
- **Honest limitation, recorded not fixed:** `npm run lint` is **not** clean repo-wide — 6 pre-existing errors in files this change does not touch (`components/AuctionHousePanel.tsx`, `components/CreateListingModal.tsx`, `lib/movementService.ts`, `scripts/e2ePlayerNotificationLive.ts`, `scripts/e2eSlice1AntiCheatLive.ts`, `scripts/spawnBots.ts`), so 0.0.32's recorded “eslint 0” is stale for the repo as a whole. Both new files lint clean.

### Fixed — Gate 3 (Mongo eradication) could never pass: it refused every push whose scan was CLEAN (FID-20260924-002 §2 findings 4–7, fixed in `e28731b`)

- **Found by the new gate's own drill, not by a review.** Wiring Gate 7 meant running the whole pre-push chain for the first time in this session — and it stopped at Gate 3 with `❌ pre-push: mongo census enumeration failed (grep exit 0) — refusing the push`, exit 2, on a tree with **zero** Mongo references.
- **The exit codes were inverted.** `grep` exits **1 for “no matches”** — precisely this gate's pass state — and the old guard treated any non-zero exit as an enumeration failure, so a clean tree was refused with exit 2 while matches fell through to the violation branch. Gate 3 could not pass at all. It now captures grep's status before any other command and refuses only on `>1`, with the contract written into the hook (`0 = matches · 1 = clean · >1 = real failure`).
- **Its own diagnostic named the opposite of the cause:** the refusal printed `grep exit 0` beside a message triggered by exit 1, because the block's first `echo` had already reset `$?`. A pass impersonating a tool failure — the mirror of the defect class hardened out of these hooks on 2026-09-16.
- **Corroboration, observable from outside:** `refs/remotes/origin/main` is still `25e8078` (2026-09-17) while Gate 3's own header dates it 2026-09-19 — `main` has not reached the remote since. (A `--no-verify` push would not show here, so this is corroboration, not proof.) After the fix the same chain prints `✅ pre-push: mongo eradication census clean` and continues to Gate 2, ending `HOOK EXIT=0` with all seven gates green.
- Indexed as its own SCOPE row (120) rather than buried in the gate's row: a gate that cannot pass is a worse failure than the drift it was written to catch.

## [0.0.32] — 2026-09-23/24 session

### Changed — FID-20260923-001: the naive-timestamp class is eliminated — 135 columns to `timestamptz`, plus the two open retaliation calls (closed, commit `19a51e2`; retaliation decisions in `0e4c468`)

- **"Which instant does this column hold?" was unanswerable, and the answer changed by host.** Of 165 timestamp columns, **135 were `timestamp without time zone`**. A value written by the application (node-pg serializes a JS `Date` in process-local wall clock) and a value written by the database (`defaultNow()` = UTC wall clock) meant **different instants in the same column**, because the Node process runs `America/New_York` while the DB session is `UTC`. The app and the DB therefore disagreed by the process offset — 4 hours, DST-variable — **in opposite directions depending on who wrote the row**, which is why it read as two unrelated oddities instead of one bug. FID-20260919-018 found one instance of the class (the clan WMD cooldown, migration 0039); this eliminates the class.
- **Migration 0040 converts all 135 columns** to `timestamptz`, interpreting each stored wall clock with the zone it was **written** in — **119 `America/New_York`** (app-written), **16 `UTC`** (DB-defaulted, `DEFAULT now()`), 57 carrying data, one guarded `DO` block applying `USING (col AT TIME ZONE zone)` per tuple. Zone **names**, not fixed offsets, so DST resolves per stored value including pre-transition history. The guard is load-bearing, not cosmetic: `timestamptz AT TIME ZONE z` yields a naive *local* timestamp, so re-applying the expression to an already-converted column would silently re-shift it.
- **134 schema declarations flipped** to `{ withTimezone: true }` across 14 files by an idempotent codemod; the six **MIXED** (multi-writer) columns carry explicit, evidence-recorded zone decisions. The conversion is lossless for app-visible behaviour — proven live **before** the plan was written: for app-written columns `AT TIME ZONE 'America/New_York'` reproduces *exactly* the instant node-pg already handed the app; for DB-defaulted columns the UTC conversion is the correct one and the NY conversion is the 4h error. Once a column is `timestamptz`, `defaultNow()` becomes correct — those five self-heal in the write direction too, no writer change needed.
- **Gate 5 installed** (`scripts/timestampConventionCensus.cjs`, fail-closed, wired into `.githooks/pre-push`): no naive instants, no `defaultNow()` over a naive column. Law 17 governed table *consumers*; nothing governed column *types*, which is how this class re-created itself after 0032 and 0039.
- **The naive fix was falsified by probe, not by argument.** Pinning `TZ=UTC` and leaving the schema alone harmonizes the 5 DB-defaulted columns and simultaneously breaks the ~130 app-written ones — trading 5 columns of error for 130. It is correct only *after* the migration, so it is deliberately **not** done here (optional determinism, not part of the class elimination; leaving it out keeps the change reversible).
- **Both retaliation calls settled as signed.** The unreviewed `RETALIATION_WINDOW = 30 * 24 * HOUR` is now the recorded **7 days** (proportionate to the 24–72h cooldowns the right bypasses), and retaliation rights finally have an **affordance**, not just an effect: `getPlayerWmdStatus()` feeds the missiles GET and `WMDMissilePanel` shows the clan WMD cooldown plus each live retaliation right with its target and expiry.
- Evidence: dev DB verified **165/165 timestamptz, 0 naive**; live instant-preservation driver **10/10** (app-local columns keep the exact instant the app computed, the DB-default column lands on its UTC-wall instant, the `timestamptz` control untouched). Gates: suite 1291/1291, tsc 0, eslint 0, all censuses exit 0.
- **Honest limitation, recorded in the FID:** the `America/New_York` calibration is correct for the environment that wrote this data (this DB is a fixture — 105/107 `players` rows are seeded). A database whose naive rows were written by a UTC process must use `UTC` for those rows; the migration header and the classifier report both say so.

### Changed — FID-20260923-002: game time is resolved in an explicit zone, and the suite is host-independent (closed, commit `200f4c3`; host-independence closure in `106af8c`; Gate 6 in `2e2091a`)

- **The second half of the same root cause.** -001 fixed where instants are **stored**; this fixes where the app decides what a game day or game hour **is**. Server code read the host process timezone — `getHours()`, `getDay()`, `setDate()`, `setHours(0,0,0,0)`, `new Date(y,m,d)` — so the same schedule fired at a different wall-clock hour depending on where the process ran, and shifted again across DST. A dev box in `America/New_York` and a UTC production host disagreed about when the week ends by four or five hours.
- **One module owns the zone.** `lib/gameTime.ts` (`gameParts`, `gameHour`, `gameDayOfWeek`, `gameDateKey`, `startOfGameDay`, `atGameTime`, `addGameDays`, `nextGameOccurrence`, `daysAgo`, `hoursAgo`) with `GAME_TIMEZONE = America/New_York`, chosen so existing behaviour on the original host is **preserved exactly** while other hosts now match it. Ten scheduled sites rewired — beer respawn **including the second, duplicated implementation** in `lib/wmd/jobs/beerBaseRespawner.ts`, the bot "Sunday 4 AM" check, the harvest reset boundary and countdown, the AM/PM raid-period guard, daily territory income, the clan-distribution reset, the chat month bucket, the economy day — and ten `setDate(getDate()-N)` retention/expiry walks normalised to exact-ms arithmetic. Gate 6 (`scripts/hostTimezoneCensus.cjs`, fail-closed, exit 2 on scan error, client display deliberately exempt) is wired beside Gate 5.
- **Running the suite in a foreign zone found three real failures** — all in `lib/beerBaseService.test.ts`, all *host-local assertions* in otherwise-correct production code (`4 AM game time` read as `8` on UTC, exactly the NY offset). The suite zone is now pinned in `vitest.setup.ts` (`TEST_TZ` override, so any zone can still be audited), and the pin was **drilled**, not assumed: reverting one assertion to the host accessor fails on this NY host (`expected 8 to be 4`) and passes under `TEST_TZ=America/New_York`.
- **Two tests were mirroring production instead of testing it.** `baseRaidFidelity` carried a hand-copied duplicate of the route's raid-period helper whose own comment claimed it would break on drift — it could not, being self-consistent in every zone. The helper is now `lib/raidPeriod.ts`, imported by **both** route and test, and injected drift fails the shared test. `stripeVipKeying` mirrored `extendVIP` with a `setDate` walk where production adds an exact duration; the divergence was unobservable across a DST transition until a new absolute DST-crossing case pinned it.
- **No single zone detects everything, so the audit is the union.** Pinning to UTC removes DST entirely, so a DST-sensitive divergence cannot appear there; `npm run test:zones` (`scripts/zoneAudit.cjs`) runs the whole suite under each zone and exits non-zero if either fails. Suite green **1301/1301 (135 files)** under UTC, `America/New_York` **and** `Asia/Tokyo`; four censuses exit 0.
- **Second-order finding, recorded not silently fixed:** `lib/activityLogService.cleanupOldLogs` computed `adminCutoffDate` and never read it, so `policy.adminLogDays` had **no effect** and admin logs were never purged. The dead arithmetic is removed and the defect recorded; implementing the purge is a data-retention product call for the operator.
- **Staging note (the harness earned its keep):** the 73-file stream was committed as 7 path-scoped commits and **each commit's tree was materialized and gated on its own**. That found three defects a single green run at HEAD would have hidden: the timestamptz commit originally carried tests importing symbols from the retaliation commit that came *after* it (tsc fail + 3 failures — fixed by reordering); the new row-count pin silently read **0 of 135** because `sql.split('\n')` met CRLF line endings on this checkout (green only on the LF working copy — fixed with CRLF normalization, recorded in the test); and `gameTimeCallSites.test.ts` shelled out to a census script parked in a later commit, leaving two trees with exactly one failure each (fixed by keeping a convention's gate in the commit that establishes it). All three are the same lesson: **a tree is not green because HEAD is green.**

### Changed — protocol: the FID status vocabulary gains `implemented` and `no-action`; archival moves to terminal statuses (FID-20260924-001, closed, commit `43ab259`)

- **Two real states could not be described.** `closed` requires a commit hash (G2), so the ledger had no lawful value for *implementation complete, gates green, commit outstanding* — and the accurate word, `implemented`, was reserved as a **legacy synonym for `closed`**, i.e. it read as an unlawful closure claim whenever a hash was still outstanding. FID-20260923-001 and -002 therefore spent a day parked on `verified`, whose spec meaning is *partially-executed work*, and each had to record the misfit in its own §8 rather than use an honest word. The spec was self-contradictory on top of that: its lifecycle diagram already named an `Implemented` stage the status list omitted.
- **`implemented` is now a first-class status** — the implementation exists and gates pass, the G2 commit is outstanding, it is **not** archival-eligible, and `closed` stays strictly hash-gated. Deliberately the same word as the existing *step* status (`scope.step_statuses`, "Code exists, gates pass"): same meaning at a different scope — a step inside a FID's plan versus a FID inside the ledger.
- **`no-action` is a new terminal status** for a documented finding or disposition with **zero code delta**, requiring a recorded disposition, evidence and a SCOPE row. It is explicitly *not* a route around implementing approved work: work that did not happen stays `blocked` (needs operator input) or `deferred` (operator-approved). `FID-20260917-005` (shrine-extend, premise dissolved) is retired under it, prior value preserved, with SCOPE row 78 named as the backing record — it had been the **only** file in `dev/fids/` since the 09-23 FIDs closed, which is what an empty "work in flight" directory exposed.
- **Archival now happens at a terminal status** (`closed` or `no-action`) — never at `loop-complete` or `implemented`. Amended in all three vocabulary surfaces (`dev/echo-v0.1.2-single-agent.md`, `protocol.config.yaml`, `templates/FID-TEMPLATE.md`) and verified to read identically, with the YAML re-parsed to prove the config still loads. Both new statuses have a live instance on the day they were added: the amending FID used `implemented` while awaiting its own commit, and `FID-20260917-005` used `no-action` on archival.
...
- Evidence: 11 pins + **23/23** live probes. The probe covers the service and the launch gate (real `launchMissile` refuses a cooldown-clan launch; a retaliation right then lets the victim's launch through and is consumed) **and** the integrated leg: a due missile driven through the tracker's own `processDueMissiles()` detonates and applies the cooldown, research penalty, relations, retaliation rights and launcher notification — proving the impact hook fires in the live path rather than only that the service works in isolation. Suite 1285/1285, tsc 0, eslint clean; census 57 tables — 57 live, 0 ticketed, 0 violations.
